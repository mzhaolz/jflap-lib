...
public class LRParseDerivationController extends LLParseDerivationController {
     ...
     */
    public LRParseDerivationController(Grammar grammar,
            Grammar augmented,
     ...
     *
     * @return <CODE>true</CODE> if the current step is finished,
     *         <CODE>false</CODE> plus some user output if the current step is
     *         unfinished
     */
    boolean done() {
        switch (step) {
            ...
            case FOLLOW_SETS:
                return super.done();
            case BUILD_DFA:
                Iterator<Map.Entry<Set<Production>, State>> it = itemsToState.entrySet().iterator();
                SelectionDrawer drawer = (SelectionDrawer) editor.getDrawer();
                int selected = 0;
                while (it.hasNext()) {
                    Map.Entry<Set<Production>, State> entry = it.next();
                    Set<Production> items = entry.getKey();
                ...
                if (selected != 0) {
                    ...
                    JOptionPane.showMessageDialog(firstFollow,
                            "The indicated states need more transitions.", "Set Not Fully Expanded",
                ...
                if (selected != 0) {
                    ...
                    JOptionPane.showMessageDialog(firstFollow,
                            ...
                            "States in Wrong Finality", JOptionPane.ERROR_MESSAGE);
                    drawer.clearSelected();
                    editor.repaint();
                    return false;
                }
            ...
            case PARSE_TABLE:
                ...
                for (int i = 0; i < rows; i++)
                    for (int j = 0; j < columns; j++)
                        if (!targetParseTable.getValueAt(i, j)
                                .equals(userParseTable.getValueAt(i, j))) {
                            highlighted++;
                            tableView.highlight(i, j);
                        }
                if (highlighted == 0)
                    return true;
                JOptionPane.showMessageDialog(firstFollow, "Highlighted cells are incorrect.",
                        "Bad Parse Table", JOptionPane.ERROR_MESSAGE);
                tableView.dehighlight();
                return false;
            case FINISHED:
                JOptionPane.showMessageDialog(firstFollow, "The parse table is complete.",
    ...
    public void completeStep() {
        switch (step) {
            ...
            case PARSE_TABLE:
                ...
                for (int i = 0; i < rows; i++)
                    for (int j = 0; j < columns; j++)
                        userParseTable.setValueAt(targetParseTable.getValueAt(i, j), i, j);
                nextStep();
                break;
            case FINISHED:
                JOptionPane.showMessageDialog(firstFollow, "The parse table is complete.",
    ...
    public void completeSelected() {
        switch (step) {
            ...
            case BUILD_DFA:
                JOptionPane.showMessageDialog(firstFollow,
                        "That request is invalid for this particular step.", "Nothing Selectable",
                        JOptionPane.ERROR_MESSAGE);
                break;
            case PARSE_TABLE:
                int rows = targetParseTable.getRowCount();
    ...
    private void completeDFA() {
        ...
        Set<State> originalStates = new HashSet<State>(Arrays.asList(states));
        while (states.length != handledStates.size()) {
            for (int i = 0; i < states.length; i++) {
                if (handledStates.contains(states[i]))
                    continue;
                Set<Production> itemSet = stateToItems.get(states[i]);
                ...
                Set<String> mayAdd = new TreeSet<String>(
                ...
                while (it.hasNext()) {
                    ...
                    State second = itemsToState.get(gotoSet);
                    if (second == null) {
                        Point p = placer.getPointForState(dfa);
                        second = dfa.createState(p);
                        Production[] gotoArray = gotoSet
     ...
     *            as; this may be <CODE>null</CODE> if the user has yet to input
     *            this set
     */
    public void gotoGroup(State first, Point point, State second) {
        String symbol = JOptionPane.showInputDialog(firstFollow,
                "What is the grammar symbol for the transition?");
        if (symbol == null)
            return;
        Set<Production> from = stateToItems.get(first);
        Set<Production> to = Operations.goTo(augmented, from, symbol);
        // Does this group even progress on this symbol?
        if (to.size() == 0) {
            JOptionPane.showMessageDialog(firstFollow, "That symbol is invalid for this state.",
        ...
        if (second != null) {
            ...
            Set<?> toUser = stateToItems.get(second);
            if (!to.equals(toUser)) {
                JOptionPane.showMessageDialog(firstFollow,
                        "The symbol " + symbol + " can't join these two states.", "Bad Progression",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
        } else {
            // We must ask the user what the second group is.
            Production[] items = itemChooser.getItemSet(to, "Goto on " + symbol);
            if (items == null)
                return;
            Set<Production> itemSet = new HashSet<Production>();
    ...
    public boolean nextStep() {
        ...
        switch (step) {
            case FIRST_SETS:
                ...
                directions.setText("Define FIRST sets.  ! is the lambda character.");
                break;
            case FOLLOW_SETS:
                firstFollow.getFFModel().setCanEditFirst(false);
                firstFollow.getFFModel().setCanEditFollow(true);
                directions.setText("Define FOLLOW sets.  $ is the end of string character.");
                break;
            case BUILD_DFA:
                doSelectedAction.setEnabled(false);
                firstFollow.getFFModel().setCanEditFollow(false);
                int choice = doAll ? JOptionPane.NO_OPTION
                        : JOptionPane.showConfirmDialog(firstFollow,
                                ...
                                "Initial Set Construction", JOptionPane.YES_NO_OPTION);
                ...
                while (initials == null) {
                    ...
                    if (initials != null)
                        break;
                    JOptionPane.showMessageDialog(firstFollow,
                            "The initial set MUST be created now.", "Initial Set Needed",
                            JOptionPane.ERROR_MESSAGE);
                }
            ...
            case PARSE_TABLE:
                ...
                targetParseTable = LRParseTableGenerator.generate(augmented, dfa, stateToItems,
                        itemsToState, Operations.follow(grammar));
            ...
            case FINISHED:
                ...
                directions.setText("Parse table complete.  Press \"parse\" to use it.");
                break;
        }
        return true;
    }
     ...
     */
    public void parse() {
        LRParsePane panel = new LRParsePane(environment, augmented, userParseTable);
    ...
    private Map<State, Set<Production>> stateToItems = new HashMap<State, Set<Production>>();

    /** The mapping of item sets to a state. */
    private Map<Set<Production>, State> itemsToState = new HashMap<Set<Production>, State>();
