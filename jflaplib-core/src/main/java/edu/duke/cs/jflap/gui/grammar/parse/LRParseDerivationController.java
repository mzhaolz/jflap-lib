...
public class LRParseDerivationController extends LLParseDerivationController {
  ...
  private boolean isFinalSet(Set<Production> set) {
    ...
    while (it.hasNext()) {
      ...
      if (p.getRHS().endsWith(GOTO_SYMBOL)) return true;
    }
    return false;
  }
  ...
  boolean done() {
    switch (step) {
      ...
      case PARSE_TABLE:
        ...
        for (int i = 0; i < rows; i++)
          for (int j = 0; j < columns; j++)
            if (!targetParseTable.getValueAt(i, j).equals(userParseTable.getValueAt(i, j))) {
              highlighted++;
              tableView.highlight(i, j);
            }
        if (highlighted == 0) return true;
        JOptionPane.showMessageDialog(
            firstFollow,
            "Highlighted cells are incorrect.",
            "Bad Parse Table",
            JOptionPane.ERROR_MESSAGE);
      ...
      case FINISHED:
        JOptionPane.showMessageDialog(
            firstFollow, "The parse table is complete.", "Finished", JOptionPane.ERROR_MESSAGE);
      default:
        return false;
    }
  }
      ...
      case PARSE_TABLE:
        ...
        for (int i = 0; i < rows; i++)
          for (int j = 0; j < columns; j++) {
            ...
            if (!tableView.isCellSelected(i, cv)) continue;
            userParseTable.setValueAt(targetParseTable.getValueAt(i, j), i, j);
          }
        tableView.repaint();
        break;
  ...
  private void completeDFA() {
    ...
    while (states.length != handledStates.size()) {
      for (int i = 0; i < states.length; i++) {
        if (handledStates.contains(states[i])) continue;
        ...
        Set<String> mayAdd = new TreeSet<String>(Arrays.asList(Operations.getCanGoto(itemSet)));
        for (int j = 0; j < t.length; j++) mayAdd.remove(((FSATransition) t[j]).getLabel());
  ...
  public void gotoGroup(State first, Point point, State second) {
    ...
    if (second != null) {
      ...
      if (!to.equals(toUser)) {
        JOptionPane.showMessageDialog(
            firstFollow,
            "The symbol " + symbol + " can't join these two states.",
            "Bad Progression",
            JOptionPane.ERROR_MESSAGE);
        return;
      }
  ...
  public boolean nextStep() {
    ...
    switch (step) {
      ...
      case PARSE_TABLE:
        ...
        targetParseTable =
            LRParseTableGenerator.generate(
                augmented, dfa, stateToItems, itemsToState, Operations.follow(grammar));
