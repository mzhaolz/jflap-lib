...
public class ChomskyPane extends JPanel {
  ...
  private void initView() {
    ...
    JSplitPane mainSplit =
        SplitPaneFactory.createSplit(
            environment, true, 0.4, new JScrollPane(grammarTable), rightPanel);
  ...
  private int[] getWhatNeedsDone() {
    ...
    for (int i = 0; i < ret.length; i++) ret[i] = ((Integer) list.get(i)).intValue();
    return ret;
  }
  ...
  private void export() {
    ...
    try {
      ...
      JOptionPane.showMessageDialog(
          this, e.getMessage(), "Export Error", JOptionPane.ERROR_MESSAGE);
      return;
    }
    try {
  ...
  public Grammar getGrammar() {
    ...
    try {
      ...
      JOptionPane.showMessageDialog(
          this, e.getMessage(), "CNF Conversion Error", JOptionPane.ERROR_MESSAGE);
      return null;
    }
    try {
      Grammar g = (Grammar) grammar.getClass().newInstance();
  ...
  private void convertSelected() {
    if (!convertAction.isEnabled()) return;
    ...
    if (unneeded > 0) {
      JOptionPane.showMessageDialog(
          this,
          "Conversion unneeded on "
              + unneeded
              + " production(s).\n"
              + (r.length - unneeded)
              + " production(s) converted.",
          "Conversion Unneeded",
    ...
    for (int i = 0; i < list.size(); i++) {
      ...
      if (last != -1 && last != toHighlight - 1) adjust++;
      last = toHighlight;
      toHighlight -= adjust;
  ...
  private void initEditingGrammarTable() {
    editingGrammarView.addMouseListener(
        new MouseAdapter() {
