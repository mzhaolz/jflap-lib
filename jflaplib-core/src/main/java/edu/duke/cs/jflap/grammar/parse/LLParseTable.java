...
public LLParseTable(LLParseTable table) {
  ...
  public boolean equals(Object object) {
    try {
      ...
      if (!Arrays.equals(terminals, other.terminals)) return false;
      for (int i = 0; i < variables.length; i++)
        if (!Arrays.equals(entries[i], other.entries[i])) return false;
      return true;
  ...
  public int getRow(String variable) {
    ...
    if (row < 0) throw new IllegalArgumentException(variable + " is not a variable!");
    return row;
  }
  ...
  public int getColumn(String lookahead) {
    ...
    if (!lookahead.equals("$")) column = Arrays.binarySearch(terminals, lookahead);
    if (column < 0) throw new IllegalArgumentException(lookahead + " is not a terminal!");
    return column + 1;
  }
  ...
  public void clear() {
    for (int i = 0; i < entries.length; i++)
      for (int j = 0; j < entries[i].length; j++) entries[i][j].clear();
  ...
  public String getColumnName(int column) {
    if (column == 0) return " ";
    if (column == terminals.length + 1) return "$";
  ...
  public Object getValueAt(int row, int column) {
    if (column == 0) return variables[row];
