...
public class TMBruteParsePane extends BruteParsePane {
     ...
     */
    public TMBruteParsePane(GrammarEnvironment environment,
            Grammar orig,
            Grammar trim,
    ...
    public void parseInput(String string, BruteParser newParser) {
        ...
        final Timer timer = new Timer(10, new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                if (parser == null)
                    return;
                String nodeCount = "Nodes generated: " + parser.getTotalNodeCount() + "("
        ...
        parser.addBruteParserListener(new BruteParserListener() {
            public void bruteParserStateChange(BruteParserEvent e) {
                synchronized (e.getParser()) {
                    String nodeCount = e.getParser().getTotalNodeCount() + " nodes generated.";
                    String status = null;
                    switch (e.getType()) {
                        ...
                        case BruteParserEvent.REJECT:
                            ...
                            if (myModel != null) {
                                ...
                                if (environment.myObjects != null)
                                    size = environment.myObjects.size();
                                int uniqueInputs = inputs.length / size;
                                myModel.setResult(row, "Reject", null,
                                        environment.myTransducerStrings, (row % uniqueInputs) * 2);
                                parseMultiple();
                            }
                            break;
                        ...
                        case BruteParserEvent.ACCEPT:
                            ...
                            if (myModel != null) {
                                myModel.setResult(row, "Accept", null,
                                        environment.myTransducerStrings, row);
                                parseMultiple();
                            }
                            break;
                    }
