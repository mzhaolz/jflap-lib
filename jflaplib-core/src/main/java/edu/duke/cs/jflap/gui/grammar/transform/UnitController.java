...
public class UnitController {
      ...
      case VARAIBLE_GRAPH:
        {
          ...
          for (int i = 0; i < p.length; i++)
            if (ProductionChecker.isUnitProduction(p[i]))
              vdgTransitions.add(UnitProductionRemover.getTransitionForUnitProduction(p[i], vdg));
      ...
      case PRODUCTION_MODIFY:
        {
          ...
          Grammar desiredGrammar = UnitProductionRemover.getUnitProductionlessGrammar(grammar, vdg);
          p = desiredGrammar.getProductions();
          for (int i = 0; i < p.length; i++) desiredProductions.add(p[i]);
          updateDisplay();
  ...
  public void doStep() {
    switch (step) {
      ...
      case PRODUCTION_MODIFY:
        ...
        pane.editingActive = false;
        Production[] p = (Production[]) desiredProductions.toArray(new Production[0]);
        for (int i = 0; i < p.length; i++) {
  ...
  void stateClicked(State state, MouseEvent event) {
    if (event.isShiftDown()) {
      ...
      if (pane.vdgDrawer.isSelected(state)) pane.vdgDrawer.removeSelected(state);
