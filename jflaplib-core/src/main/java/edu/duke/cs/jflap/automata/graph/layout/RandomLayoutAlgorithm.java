...
public class RandomLayoutAlgorithm extends LayoutAlgorithm {
  ...
  public void layout(Graph graph, Set notMoving) {
    ...
    for (int i = 0; i < points.size(); i++) graph.moveVertex(chain.get(i), (Point2D) points.get(i));
    ...
    while (notProcessedPoints.size() > 0) {
      ...
      for (int i = 0; i < notProcessedPoints.size(); i++) {
        ...
        if (current.getY() != anchor.getY())
          currentTheta =
              Math.atan((current.getX() - anchor.getX()) / (current.getY() - anchor.getY()));
        else if (current.getX() > anchor.getX()) currentTheta = Math.PI / 2;
        else currentTheta = Math.PI / -2;
