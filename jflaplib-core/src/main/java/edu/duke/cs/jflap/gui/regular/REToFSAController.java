...
public class REToFSAController {
  ...
  private FSATransition[] replaceTransition(FSATransition transition, String[] exps) {
    ...
    for (int i = 0; i < exps.length; i++) {
      pStart = new Point();
      pEnd = new Point();
      double y = exps.length > 1 ? ((double) i / ((double) exps.length - 1.0) - 0.5) * 0.5 : 0.0;
      pe.y = ps.y = y;
      ...
      case DEPARENS:
        {
          ...
          if (requiredAction(newLabel) != 0) toDo.add(t);
  ...
  public void completeAll() {
    while (action != 0 || toDo.size() > 0) completeStep();
  }
  ...
  public void exportToTab() {
    edu.duke.cs.jflap.gui.viewer.AutomatonPane p =
        new edu.duke.cs.jflap.gui.viewer.AutomatonPane(automaton);
  ...
  private void nextStep() {
    if (transitionNeeded == 0) {
      if (toDo.size() > 0) {
        if (action != 0) convertPane.mainLabel.setText("Resolution complete.");
        else convertPane.mainLabel.setText("Welcome to the converter.");
