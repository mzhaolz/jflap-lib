...
public class ConvertPDAToGrammarAction extends ConvertAutomatonToGrammarAction {
  ...
  protected boolean checkAutomaton() {
    ...
    if (bad.size() != 0) {
      ...
      while (it.hasNext()) drawer.addSelected(it.next());
      messageLabel.setText("Transitions to final must pop only 'Z'.");
      JOptionPane.showMessageDialog(
          frame, messagePanel, "Final Transitions Error", JOptionPane.ERROR_MESSAGE);
      return false;
    }
    ...
    for (int i = 0; i < transitions.length; i++) {
      ...
      t.getStringToPop().length() != 1
          || (t.getStringToPush().length() != 2 && t.getStringToPush().length() != 0)) bad.add(t);
    }
    if (bad.size() != 0) {
      drawer.clearSelected();
      Iterator<PDATransition> it = bad.iterator();
      ...
      JOptionPane.showMessageDialog(
          frame, messagePanel, "Transitions Error", JOptionPane.ERROR_MESSAGE);
      return false;
    }
    return true;
  }
