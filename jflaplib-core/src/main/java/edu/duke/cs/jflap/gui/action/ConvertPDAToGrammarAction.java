...
public class ConvertPDAToGrammarAction extends ConvertAutomatonToGrammarAction {
	 ...
	 *            the environment
	 */
	public ConvertPDAToGrammarAction(final AutomatonEnvironment environment) {
		super(environment);
	}
	...
	protected boolean checkAutomaton() {
		...
		final List<State> finalStates = getAutomaton().getFinalStates();
		if (finalStates.size() != 1) {
			JOptionPane.showMessageDialog(frame, "There must be exactly one final state!", "Final State Error",
					JOptionPane.ERROR_MESSAGE);
			return false;
		}
		// Are all transitions to the final state okay?
		final List<Transition> toFinal = getAutomaton().getTransitionsToState(finalStates.get(0));
		final HashSet<PDATransition> bad = new HashSet<>();
		for (int i = 0; i < toFinal.size(); i++) {
			final PDATransition t = (PDATransition) toFinal.get(i);
		...
		if (bad.size() != 0) {
			...
			final Iterator<PDATransition> it = bad.iterator();
			while (it.hasNext()) {
				drawer.addSelected(it.next());
			}
			messageLabel.setText("Transitions to final must pop only 'Z'.");
			JOptionPane.showMessageDialog(frame, messagePanel, "Final Transitions Error", JOptionPane.ERROR_MESSAGE);
			return false;
		}
		// Are the transitions okay?
		final List<Transition> transitions = getAutomaton().getTransitions();
		bad.clear();
		for (int i = 0; i < transitions.size(); i++) {
			final PDATransition t = (PDATransition) transitions.get(i);
		...
		if (bad.size() != 0) {
			...
			JOptionPane.showMessageDialog(frame, messagePanel, "Transitions Error", JOptionPane.ERROR_MESSAGE);
			return false;
		}
		return true;
	}
