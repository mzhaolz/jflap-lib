...
public class SimulateAction extends AutomatonAction {
	 ...
	 */
	public SimulateAction(final Automaton automaton, final Environment environment) {
		super("Step...", null);
		if (SimulateNoClosureAction.isApplicable(automaton)) {
			putValue(NAME, "Step with Closure...");
		}
		putValue(ACCELERATOR_KEY, KeyStroke.getKeyStroke(KeyEvent.VK_R, MAIN_MENU_MASK));
		this.automaton = automaton;
		this.environment = environment;
	}

	public SimulateAction(final Grammar gram, final Environment environment) {
		super("Step...", null);
		if (SimulateNoClosureAction.isApplicable(automaton)) {
			putValue(NAME, "Step with Closure...");
		}
	...
	public void actionPerformed(final ActionEvent e) {
		...
		/*
		 * if (input instanceof InputBox) { InputBox tt=(InputBox) input; if
		 * (getObject() instanceof TuringMachine) tt.addSimulator(automaton,
		 * simulator, true); else tt.addSimulator(automaton, simulator, false);
		 * return; }
		 */
		if (input == null) {
			return;
	...
	/**
	 * Returns whether the current automaton can legally be simulated. If not, a
	 * dialog pops up. Will true if <code>automaton</code> = <i>null</i>.
	 *
	 * @param source
	 *            the source of this action
	 * @return whether the current automaton can legally be simulated
	 */
	protected boolean automatonActionPermissible(final Component source) {
		...
		/*
		 * If it is a Turing machine, there are transitions from the final
		 * state, and that preference hasn't been enabled, give a warning and
		 * return.
		 */
		else if (automaton instanceof TuringMachine && !Universe.curProfile.transitionsFromTuringFinalStateAllowed()) {
			final TuringMachine turingMachine = (TuringMachine) automaton;
			final List<State> finalStates = turingMachine.getFinalStates();
			final AutomatonDirectedGraph graph = new AutomatonDirectedGraph(turingMachine);
			for (int i = 0; i < finalStates.size(); i++) {
				if (graph.fromDegree(finalStates.get(i), false) > 0) {
					JOptionPane.showMessageDialog(source,
	...
	private void handleInputFile(final Object input) {
		List<Configuration> configs = null;
		final AutomatonSimulator simulator = getSimulator(automaton);
		/*
		 * if (input instanceof InputBox) { InputBox tt=(InputBox) input; if
	...
	public void handleInteraction(final Automaton automaton, final AutomatonSimulator simulator,
			final List<Configuration> configs, Object initialInput) {
		final SimulatorPane simpane = new SimulatorPane(automaton, simulator, configs, environment, false);
	...
	protected Object initialInput(final Component component, String title) {
		...
		if (getObject() instanceof TuringMachine) {
			...
			final TuringMachine tm = (TuringMachine) getObject();
			int tapes = tm.tapes();
	...
	private Object openInputGUI(final Component component, final String title, final int tapes) {
		...
		final JButton jb = new JButton("Click to Open Input File");
						...
						for (int i = 0; i < tapes; i++) {
							if (sc.hasNext()) {
								...
								JOptionPane.showMessageDialog(component,
										"Input file does not have enough input for all tapes", "File read error",
		...
		panel.add(jb);
		final int result = JOptionPane.showOptionDialog(component, panel, title, JOptionPane.OK_CANCEL_OPTION,
	...
	protected void setEnvironment(final Environment newEnv) {
		environment = newEnv;
	}
}
