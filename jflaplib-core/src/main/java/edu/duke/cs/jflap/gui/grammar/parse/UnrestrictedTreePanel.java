...
private HashMap<String, String> myVariableMap;
  ...
  private void bridgeTo(int level) {
    ...
    for (int i = 0; i < prev.length; i++) {
      if (prodNum >= prods.length
          || length < prodStarts[prodNum]
        ...
        for (int j = 0; j < rhs.length(); j++) {
          ...
          currentTop.add(node);
          if (j == rhs.length() - 1) nodeToParentGroup.put(node, b);
        }
  ...
  private boolean ends(int level, int group) {
    try {
      if (level == bottom.length - 1) return true; // Everything ends at last.
  ...
  private boolean assignWeights(int level, boolean[] need) {
    ...
    for (int i = 0; i < bottom[level].length; i++) {
      ...
      if (cSum > sSum) {
        ...
        for (int j = 0; j < s.length; j++) s[j].weight *= ratio;
        if (level != 0) need[level - 1] = true;
        changed = true;
      } else if (cSum < sSum) {
        double ratio = sSum / cSum;
        for (int j = 0; j < c.length; j++) c[j].weight *= ratio;
        if (level != 0) need[level + 1] = true;
        changed = true;
      }
    }
  ...
  public void setAnswer(ParseNode answer) {
    ...
    for (int max = 0; changed && max < top.length * 2; max++) {
      changed = false;
      for (int i = 0; i < top.length - 1; i++) changed |= assignWeights(i, need);
    }
    level = group = 0;
    brutePane.derivationModel.setRowCount(0);
    brutePane.derivationModel.addRow(new String[] {"", solutionParseNodes[0].getDerivation()});
    return;
  }
  ...
  public void paintNode(Graphics2D g, UnrestrictedTreeNode node, Point2D p) {
    ...
    if (myVariableMap == null) nodeDrawer.draw(g, node);
    else {
  ...
  protected Point2D getPoint(int row, double weight, Point2D p) {
    ...
    p.setLocation(realWidth * weight / metaWidth, realHeight * ((double) row + 0.5) / metaHeight);
    return p;
  }
  ...
  protected void setMetaWidth() {
    for (int i = 0; i < top.length; i++) {
      ...
      double total = 0.0;
      if (nodes != null) {
        for (int j = 0; j < nodes.length; j++) total += nodes[j].weight;
      }
  ...
  public boolean next() {
    ...
    do {
      ...
      if (level == top.length - 1 && group == top[level].length - 1) break;
    } while (!begins(level, group));
    String lhs = p.getRHS();
    if (lhs.length() == 0) lhs = Universe.curProfile.getEmptyString();
