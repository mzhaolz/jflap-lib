...
class LRParseController {
  ...
  private String derivationString() {
    StringBuffer sb = new StringBuffer();
    for (int i = 1; i < STACK.size(); i += 2) sb.append(STACK.get(i));
  ...
  public void step() {
    ...
    if (reduceStep == 1) state = reduceState;
    ...
    if (entry.length() == 0 || Character.isDigit(entry.charAt(0))) {
      ...
      if (reduceStep == 0) {
        ...
        if (popped.length() == 0) popped = Universe.curProfile.getEmptyString();
        ...
        STACK.push(Integer.parseInt(pane.table.getValueAt(state, red.getLHS())));
        pane.treeDrawer.show(nodes[NODECOUNT++]);
        pane.derivationModel.addRow(new String[] {red.toString(), derivationString()});
        reduceStep = 0;
      }
      pane.statusDisplay.setText(displayString);
      pane.treePanel.repaint();
    ...
    while (true) {
      ...
      if (entry.length() == 0 || Character.isDigit(entry.charAt(0))) {
        ...
        for (int i = 0; i < elements.length; i++)
          if (elements[i] instanceof MutableTreeNode) node.add((MutableTreeNode) elements[i]);
        ...
        if (red.getRHS().length() == 0) {
          // Lambda.
          MutableTreeNode lambda = new DefaultMutableTreeNode(Universe.curProfile.getEmptyString());
      ...
      if (numberOfIterations >= numberTillNextWarning) {
        int result =
            JOptionPane.showConfirmDialog(
                pane, numberOfIterations + " nodes have been generated.  Should we continue?");
