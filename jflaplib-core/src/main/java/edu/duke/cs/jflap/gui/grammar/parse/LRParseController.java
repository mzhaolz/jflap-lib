...
class LRParseController {
     ...
     * @param table
     *            the parse table
     * @return the parse tree, or <CODE>null</CODE> if the user cancelled due to
     *         it taking too long
     */
    private DefaultTreeModel parseTree(String string, Grammar grammar, LRParseTable table) {
        ...
        while (true) {
            ...
            if (numberOfIterations >= numberTillNextWarning) {
                int result = JOptionPane.showConfirmDialog(pane,
                        numberOfIterations + " nodes have been generated.  Should we continue?");
