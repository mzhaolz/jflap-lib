...
public class RestrictedUserParser extends UserParser {
  ...
  public boolean isPossibleDerivation(String derivation) {
    ...
    if (derivation.length() == 0) {
      ...
      endBookend =
          !myGrammar.isVariable(derivation.substring(derivation.length() - 1, derivation.length()));
    }

    /* Break up groups of terminals into the "discrete" array. */
    for (int i = 0; i <= derivation.length(); i++) {
      String symbol = i == derivation.length() ? null : derivation.substring(i, i + 1);
    ...
    for (int i = 0; i < discrete.size(); i++) {
      String e = discrete.get(i);
      if (startBookend && i == 0) {
        if (!myTarget.startsWith(e)) return false;
        cp = e.length();
      } else if (endBookend && i == discrete.size() - 1) {
        if (!myTarget.endsWith(e)) return false;
      } else {
