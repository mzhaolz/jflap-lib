...
/**
 * RestrictedUser Parser (This class is very similar to RestrictedBruteParser
 * except for that this class extends UserParser) May be important in the future
 *
...
public class RestrictedUserParser extends UserParser {
	 ...
	 *            the target string
	 */
	public RestrictedUserParser(final Grammar grammar, final String target) {
	...
	public boolean isPossibleDerivation(final String derivation) {
		...
		final ArrayList<String> discrete = new ArrayList<>();
		...
		if (derivation.length() == 0) {
			...
			endBookend = !myGrammar.isVariable(derivation.substring(derivation.length() - 1, derivation.length()));
		}

		/* Break up groups of terminals into the "discrete" array. */
		for (int i = 0; i <= derivation.length(); i++) {
			final String symbol = i == derivation.length() ? null : derivation.substring(i, i + 1);
