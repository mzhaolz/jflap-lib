...
public class PumpingLemmaChooserPane extends JPanel {
  ...
  private void init() {
    ...
    for (int i = 0; i < myChooser.size(); i++) listPanel.add(addPumpingLemma(i));
  ...
  private JPanel initRadioButtonPanel() {
    ...
    buttonPanel.setBorder(
        BorderFactory.createTitledBorder("First choose who makes the first move."));
  ...
  private JPanel addPumpingLemma(int i) {
    ...
    JEditorPane ep =
        new JEditorPane(
            "text/html",
            "<html><body align=center><b><i>L</i> = {"
                + lemma.getHTMLTitle()
                + "}</b></body></html>");
    ...
    PumpingLemmaChooseButton button =
        new PumpingLemmaChooseButton(myChooser.get(i), myEnvironment, i);
