...
public class TMTransitionCreator extends TableTransitionCreator {
  ...
  protected Transition initTransition(State from, State to) {
    if (!blockTransition) return initTransition(from, to, "R");
    else return initTransition(from, to, "S");
  }
  ...
  protected Transition initTransition(State from, State to, String directionString) {
    ...
    for (int i = 0; i < read.length; i++) read[i] = "";
    String[] write = read;
    if (blockTransition) {
    ...
    for (int i = 0; i < direction.length; i++) direction[i] = directionString;
  ...
  protected TableModel createModel(Transition transition) {
    ...
    return new AbstractTableModel() {
      ...
      public boolean isCellEditable(int r, int c) {
        if (!blockTransition) return true;
      ...
      directionColumn.setCellEditor(
          new DefaultCellEditor(BOX) {
            ...
            public Component getTableCellEditorComponent(
              ...
              for (int i = 0; i < STROKES.length; i++) imap.put(STROKES[i], o);
              return c;
            }
          });
    }
  ...
  public Transition modifyTransition(Transition transition, TableModel model) {
    ...
    try {
      ...
      TMTransition newTrans =
          new TMTransition(t.getFromState(), t.getToState(), reads, writes, dirs);
  ...
  public static void setDirs(boolean allowStay) {
    if (allowStay) DIRS = new String[] {"R", "S", "L"}; // made this non-static to
    // allow for switching option
    else {
    ...
    BOX.removeAllItems();
    for (int i = 0; i < DIRS.length; i++) BOX.addItem(DIRS[i]);
  }
