...
public abstract class TableTransitionCreator extends TransitionCreator {
	...
	private static class TipLambdaCellRenderer extends LambdaCellRenderer {
		...
		public TipLambdaCellRenderer(final String tip) {
			setToolTipText(tip);
		}

		public TipLambdaCellRenderer(final String replace, final String tip) {
			super(replace);
			setToolTipText(tip);
		}
	}
	...
	private final MouseListener viewListener = new MouseAdapter() {
		@Override
	 ...
	 */
	public TableTransitionCreator(final AutomatonPane parent) {
		super(parent);
	...
	protected JTable createTable(final Transition transition) {
		final TableModel model = createModel(transition);
		...
		for (int i = 0; i < model.getColumnCount(); i++) {
			renders[i] = transition instanceof TMTransition
					? new TipLambdaCellRenderer("" + Tape.BLANK, model.getColumnName(i))
					: new TipLambdaCellRenderer(model.getColumnName(i));
		}
		final JTable table = new JTable(createModel(transition)) {
			/**
			 *
			 */
			private static final long serialVersionUID = 1L;
			...
			protected boolean processKeyBinding(final KeyStroke ks, final KeyEvent e, final int condition,
				...
				if (ks.getKeyCode() == KeyEvent.VK_ENTER && !ks.isOnKeyRelease()) {
					stopEditing(false);
					if (e.isShiftDown()) {
						createTransition(transition.getFromState(), transition.getToState());
					}
					return true;
	...
	public Transition createTransition(final State from, final State to) {
		final Transition t = initTransition(from, to);
		editTransition(t, null);

		// can you say "ugly hack?"
		// editTransition(t, new Point((from.getPoint().x+to.getPoint().x)/2,
	...
	@Override
	public boolean editTransition(final Transition t) {
		return false;
	}
	 ...
	 */
	@Override
	public void editTransition(final Transition transition, Point point) {
		...
		tablePoint.translate(-tableDimensions.width / 2, -tableDimensions.height / 2);
		getParent().setTablePoint(tablePoint);
	...
	protected void editTransition(final Transition transition, final Point point, final int tableSize) {
	}
	 ...
	 */
	protected abstract Transition modifyTransition(Transition transition, TableModel model);
	 ...
	 */
	private void stopEditing(final boolean cancel) {
		...
		if (!cancel) {
			...
			final Transition t = modifyTransition(transition, editingTable.getModel());
			if (t != null) {
				if (isNew) {
					getParent().getDrawer().getAutomaton().addTransition(t);
				} else {
					getParent().getDrawer().getAutomaton().replaceTransition(transition, t);
				}
			}
		}
		if (this instanceof TMTransitionCreator) {
			final TMTransitionCreator stop = (TMTransitionCreator) this;
