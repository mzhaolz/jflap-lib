...
public class ConversionController {
  ...
  public ConversionController(
      FiniteStateAutomaton nfa, FiniteStateAutomaton dfa, ConversionPane view) {
    this.nfa = nfa;
    this.dfa = dfa;
    this.view = view;
  ...
  private void registerState(State state) {
    ...
    for (Object o : setToState.keySet()) EDebug.print(o.toString());
  ...
  public void expandState(State start, Point point, State end) {
    ...
    if (terminal.equals("")) {
      JOptionPane.showMessageDialog(
          view,
          "One can't have lambda in the DFA!",
          "Improper terminal",
    ...
    if (end == null)
      userEnd =
          JOptionPane.showInputDialog(
              view, "Which group of NFA states will that go to on " + terminal + "?");
    if (userEnd == null) return;
    State[] userEndStates = endStates;
    try {
      if (end == null) userEndStates = getStatesForString(userEnd, nfa);
    ...
    if (!converter.containSameStates(userEndStates, endStates)) {
      JOptionPane.showMessageDialog(
          view, "That list of states is incorrect!", "Wrong set", JOptionPane.ERROR_MESSAGE);
      return;
    }
  ...
  public void complete() {
    ...
    while (stateQueue.size() != 0) expandState((State) stateQueue.removeFirst());
