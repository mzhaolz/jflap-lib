...
public class CYKParsePane extends BruteParsePane {
   ...
   */
  public CYKParsePane(
      GrammarEnvironment environment, Grammar original, Grammar cnf, InputTableModel model) {
  ...
  public void input(String string) {
    ...
    if (myParser.solve(string)) {
      ...
      traceBack();
    } else progress.setText("String is Rejected!");
  }
  ...
  public void parseMultiple() {
    ...
    while (row < (inputs.length - 1)) {
      ...
      else myModel.setResult(row, "Reject", null, environment.myTransducerStrings, row);
    }
  }
