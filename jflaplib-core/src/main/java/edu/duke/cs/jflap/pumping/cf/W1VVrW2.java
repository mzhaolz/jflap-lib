...
public class W1VVrW2 extends ContextFreePumpingLemma {
  ...
  public void setDescription() {
    ...
    explanation =
        "Because this is a context-free language, a valid decomposition exists.  If |'v'| "
            ...
            + "character of 'v<sup>R</sup>'.  Otherwise, if <i>m</i> "
            + GREATER_OR_EQ
            + " 8 and |v| = 3, one could just "
      ...
      for (int start = 0; start <= end - 5; start++)
        if ((end - start) % 2 == 1 && s.charAt(start) == s.charAt(end)) {
          ...
          for (int i = 0; i <= (end - start) / 2; i++)
            if (s.charAt(start + i) != s.charAt(end - i)) match = false;
          if (match
              && LemmaMath.countInstances(s.substring(0, start), 'a')
  ...
  public void chooseDecomposition() {
    ...
    else if (w1.indexOf('b') > -1) setDecomposition(new int[] {w1.indexOf('b'), 1, 0, 0});
