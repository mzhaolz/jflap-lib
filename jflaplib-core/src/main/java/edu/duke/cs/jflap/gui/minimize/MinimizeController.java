...
class MinimizeController {
     ...
     */
    public MinimizeController(MinimizePane view,
            SelectionDrawer adrawer,
            SelectTreeDrawer tdrawer,
            Minimizer minimizer) {
        this.view = view;
    ...
    void setEnabledness() {
        ...
        if (expanding == node) {
            ...
            cp.autoPartitionAction
                    .setTip("Complete the expansion of this group on " + node.getTerminal() + ".");
            ...
            cp.setTerminalAction
                    .setTip("Cannot expand another group while " + es + " is in progress.");
            cp.autoPartitionAction.setEnabled(false);
            cp.autoPartitionAction
                    .setTip("Cannot expand another group while " + es + " is in progress.");
            return;
    ...
    public boolean splitOnTerminal(MinimizeTreeNode node) {
        ...
        if (!minimizer.isSplittableOnTerminal(node.getStates(), terminal, getAutomaton(),
                getTree())) {
    ...
    public void splitWithoutInput(MinimizeTreeNode node) {
        ...
        if (node.getTerminal().equals("")) {
            ...
            // We need to get a terminal for this node!
            node.setTerminal(
                    minimizer.getTerminalToSplit(node.getStates(), getAutomaton(), getTree()));
        }
        split(node);
    }
    ...
    public void splitSubtree(MinimizeTreeNode root) {
        ...
        if (children.length == 0) {
            ...
            root.setTerminal(
                    minimizer.getTerminalToSplit(root.getStates(), getAutomaton(), getTree()));
            split(root);
