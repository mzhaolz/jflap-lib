...
public abstract class AutomatonTransducer extends AbstractTransducer {
  ...
  protected void createState(
    ...
    for (int i = 0; i < stateNodes.size(); i++) {
      ...
      if (idString == null) throw new DataException("State without id attribute encountered!");
  ...
  protected Element createTransitionElement(Document document, Transition transition) {
    ...
    te.appendChild(
        createElement(
            document, TRANSITION_FROM_NAME, null, "" + transition.getFromState().getID()));
    // Encode the to state.
    te.appendChild(
        createElement(document, TRANSITION_TO_NAME, null, "" + transition.getToState().getID()));
  ...
  protected Element createBlockElement(Document document, TMState block, Automaton container) {
    ...
    Automaton a =
        block
            .getAutomaton(); //this will fetch the parent - I see no reason to go through the block map
