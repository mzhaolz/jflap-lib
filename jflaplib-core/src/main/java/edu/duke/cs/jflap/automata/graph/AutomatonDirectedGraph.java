...
public void removeEdge(State from, State to) {
  ...
  public int fromDegree(State from, boolean excludeSameVertexEdges) {
    ...
    int count = 0;
    Set<State> vertices = verticesToNeighbors.keySet();
    for (State state : vertices) if (hasEdge(from, state) && !state.equals(from)) count++;
    return count;
  }
  ...
  public int toDegree(State to, boolean excludeSameVertexEdges) {
    ...
    for (State state : vertices)
      if (hasEdge(state, to) && (!state.equals(to) || !excludeSameVertexEdges)) count++;
    return count;
  }
}
