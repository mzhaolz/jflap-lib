...
public class Expander {
  ...
  private List<String> expandContext(List<String> symbols) {
    ...
    for (int i = 0; i < symbols.size(); i++) {
      ...
      for (int j = 0; j < contexts.size(); j++) {
        List<List<String>> l = contexts.get(j).matches(symbols, i);
        for (int k = 0; k < l.size(); k++) replacementsList.add(l.get(k));
      }
  ...
  private final void initializeContexts() {
    ...
    // Set the array.
    if (hasContexts) contexts = contextsList;
  }
  ...
  private class Context {
     ...
     */
    public Context(List<String> tokens, int center, List<List<String>> replacements) {
      this.tokens = tokens;
      this.center = center;
