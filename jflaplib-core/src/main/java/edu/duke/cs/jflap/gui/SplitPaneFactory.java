...
public class SplitPaneFactory {
	 ...
	 */
	public static JSplitPane createSplit(final Environment environment, final boolean horizontal, final double ratio,
			final Component left, final Component right) {
		final JSplitPane split = new JSplitPane(horizontal ? JSplitPane.HORIZONTAL_SPLIT : JSplitPane.VERTICAL_SPLIT,
				true, left, right);
		Dimension dim = environment.getSize();
		final Component[] comps = environment.getComponents();
		if (comps.length != 0) {
			dim = comps[0].getSize();
		}
		final int size = horizontal ? dim.width : dim.height;
