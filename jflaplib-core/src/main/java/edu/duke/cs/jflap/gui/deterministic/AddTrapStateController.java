 ...
 * transitions This class is very similar to FSATOREController.java under
 * gui.regular
 *
...
public class AddTrapStateController {
  ...
  private static final int
      CREATE_SINGLE_TRAPSTATE = 0,
      TRANSITIONS_TO_TRAPSTATE = 1,
      FINISHED = 200;
  ...
  private void nextStep() {
    switch (currentStep) {
      ...
      case TRANSITIONS_TO_TRAPSTATE:
        if (myReadSets == null) determineRemainingTransition();
        mainStep.setText("Adding Transitions    Readable String : " + myReadSets);
        detailStep.setText(
            "Put transitions from all states to the trap state.   "
                + remaining
  ...
  public void transitionCreate(State from, State to) {
    ...
    if (!to.equals(myTrapState)) {
      JOptionPane.showMessageDialog(
          frame,
          "You have to make transition to the trap state!",
          "Error",
          JOptionPane.ERROR_MESSAGE);
      return;
    }
    ...
    if (terminal.length() > 1) {
      JOptionPane.showMessageDialog(
          frame, "Terminal can only be a single letter", "Error", JOptionPane.ERROR_MESSAGE);
      return;
    }
