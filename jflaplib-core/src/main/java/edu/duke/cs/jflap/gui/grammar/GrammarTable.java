...
public class GrammarTable extends HighlightTable {
  ...
  public Grammar getGrammar(Class<?> grammarClass) {
    ...
    for (int row = 0; row < model.getRowCount(); row++) {
      ...
      try {
        ...
        if (grammar.getStartVariable() == null) grammar.setStartVariable(p.getLHS());
      } catch (IllegalArgumentException e) {
        setRowSelectionInterval(row, row);
        JOptionPane.showMessageDialog(
            this, e.getMessage(), "Production Error", JOptionPane.ERROR_MESSAGE);
        return null;
      }
    }
    return grammar;
  ...
  private static final edu.duke.cs.jflap.gui.HighlightTable.TableHighlighterRendererGenerator THRG =
