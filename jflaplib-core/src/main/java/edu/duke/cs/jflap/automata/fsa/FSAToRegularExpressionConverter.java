...
public class FSAToRegularExpressionConverter {
  ...
  public static boolean areRemovableStates(Automaton automaton) {
    ...
    for (State state : states) {
      if (isRemovable(state, automaton)) return true;
    }
    return false;
  }
   ...
   *         <CODE>expression</CODE> as the transition label.
   */
  public static Transition getTransitionForExpression(
    ...
    Transition transition = new FSATransition(fromState, toState, expression);
    return transition;
  }
   ...
   *         and <CODE>toState</CODE> in <CODE>automaton</CODE>.
   */
  public static String getExpressionBetweenStates(
      State fromState, State toState, Automaton automaton) {
    List<Transition> transitions = automaton.getTransitionsFromStateToState(fromState, toState);
    FSATransition trans = (FSATransition) transitions.get(0);
   ...
   *            the automaton.
   */
  public static void removeState(State state, List<Transition> transitions, Automaton automaton) {
   ...
   *         removing <CODE>state</CODE>.
   */
  public static List<Transition> getTransitionsForRemoveState(State state, Automaton automaton) {
   ...
   */
  public static FSATransition addTransitionOnEmptySet(
      State fromState, State toState, Automaton automaton) {
   ...
   */
  public static FSATransition combineToSingleTransition(
      State fromState, State toState, List<Transition> transitions, Automaton automaton) {
    ...
    for (int k = 0; k < states.size(); k++) {
      for (int j = 0; j < states.size(); j++) {
        List<Transition> transitions =
            automaton.getTransitionsFromStateToState(states.get(k), states.get(j));
  ...
  public static void convertToGTG(Automaton automaton) {
    ...
    for (int k = 0; k < states.size(); k++) {
      ...
      if (state != finalState && state != initialState) {
        List<Transition> transitions = getTransitionsForRemoveState(state, automaton);
