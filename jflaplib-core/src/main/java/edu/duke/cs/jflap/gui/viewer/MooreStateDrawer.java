...
/**
 * This is a special subclass of a <code>StateDrawer</code> that draws the
 * output of states for Moore machines. The output is obtained by calling
 * {@link edu.duke.cs.jflap.automata.mealy.MooreMachine#getOutput(State)}.
 *
 * @author Jinghui Lim
 *
 */
public class MooreStateDrawer extends StateDrawer {
	...
	/**
	 * Creates a <code>MooreStateDrawer</code> with states drawn to a particular
	 * radius.
	 *
	 * @param radius
	 *            the radius of the states drawn
	 */
	public MooreStateDrawer(final int radius) {
		super(radius);
	}
	 ...
	 *            the color of the inner portion of the state
	 */
	private void drawStateOutput(final Graphics g, final State state, final Point point, final Color color) {
		final String output = ((MooreMachine) state.getAutomaton()).getStateDescription(state);

		final int ascent = g.getFontMetrics().getAscent();
		int heights = 0;
		int textWidth = 0;

		final Rectangle2D bounds = g.getFontMetrics().getStringBounds(output, g);
		...
		final int x = point.x - (width >> 1) + STATE_RADIUS;
		final int y = point.y - STATE_RADIUS - height / 2;
