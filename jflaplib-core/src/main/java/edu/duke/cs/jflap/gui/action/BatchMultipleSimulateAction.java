...
public class BatchMultipleSimulateAction extends MultipleSimulateAction {
    ...
    protected JTableExtender initializeTable(Object obj) {
        ...
        if (multiple) {
            List<Object> autos = getEnvironment().myObjects;
            // System.out.println("In initialize: " + autos.size());
            List<String> strings = getEnvironment().myTestStrings;
    ...
    public void performAction(Component source) {
        ...
        if (getObject() instanceof MealyMachine) {
            ...
            List<State> nd = d.getNondeterministicStates(a);
            if (nd.size() > 0) {
                JOptionPane.showMessageDialog(source,
        ...
        bar.add(new AbstractAction("Run Inputs") {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (getObject() instanceof Automaton) {
                    Automaton currentAuto = (Automaton) getEnvironment().myObjects.get(0);
                    ...
                    for (int r = 0; r < inputs.length; r++) {
                        if (r > 0) {
                            if (r % uniqueInputs == 0) {
                                currentAuto = (Automaton) getEnvironment().myObjects
                                        .get(r / uniqueInputs);
                        ...
                        if (currentAuto instanceof TuringMachine) {
                            ...
                            configs = ((TMSimulator) simulator)
                                    .getInitialConfigurations(Arrays.asList(inputs[r]));
                            input = inputs[r];
                        ...
                        int result = handleInput(currentAuto, simulator, configs, input,
                                associated);
                        ...
                        if (getObject() instanceof MealyMachine) {
                            ...
                            model.setResult(r, con.getOutput(), con,
                                    getEnvironment().myTransducerStrings,
                                    (r % (uniqueInputs)) * (tapes + 1));
                        } else {
                            model.setResult(r, RESULT.get(result), c,
                                    getEnvironment().myTransducerStrings,
                                    (r % (uniqueInputs)) * (tapes + 1));
                        }
                    }
                    ...
                    BruteParsePane parsePane = new BruteParsePane(
                            (GrammarEnvironment) getEnvironment(), currentGram, model);
            ...
            bar.add(new AbstractAction("View Trace") {
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    for (int row : rows) {
                        ...
                        Configuration c = tm.getAssociatedConfigurationForRow(row);
                        if (c == null) {
        ...
        if (((InputTableModel) table.getModel()).isMultiple) {
            ...
            bar.add(new AbstractAction("Add input string") {
                ...
                public void actionPerformed(ActionEvent arg0) {
                    ...
                    // add expected output
                    if (turing) {
                        Object output = initialInput(getEnvironment().getActive(),
                                "Expected Output?");
                        ...
                        if (output instanceof String) {
                            ...
                            getEnvironment().myTestStrings
                                    .remove(getEnvironment().myTestStrings.size() - 1);
                            return;
                        }
                    }
                    // add expected result
                    Object result = initialInput(getEnvironment().getActive(),
            ...
            bar.add(new AbstractAction("Save Results") {
                ...
                public void actionPerformed(ActionEvent arg0) {
                    ...
                    final JRadioButton defaultLocation = new JRadioButton(
                        ...
                        if (specifyLocation.isSelected()) {
                            ...
                            while (badname || file == null) {
                                if (!badname) {
                                    ...
                                    Universe.CHOOSER
                                            .setDialogTitle("Choose directory to save files in");
                                    Universe.CHOOSER
                            ...
                            for (int r = 0; r < model.getRowCount(); r++) {
                                ...
                                if (!fileName.equals(oldfileName)) {
                                    ...
                                    bwriter = new BufferedWriter(
                                            new FileWriter(base + "results" + fileName + ".txt"));
                                ...
                                for (int c = 1; c < model.getColumnCount(); c++) {
                                    ...
                                    if ((model.getColumnName(c).startsWith("Output")) && !output
                                            && turing) {
                                        out.write("Output: ");
                                        output = true;
                                    }
                                    ...
                                    String value = model.getValueAt(r, c);
        ...
        if (finObject instanceof Automaton) {
            ...
            JSplitPane split = SplitPaneFactory.createSplit(getEnvironment(), true, 0.5, ap, panel);
            ...
            BruteParsePane bp = new BruteParsePane((GrammarEnvironment) getEnvironment(),
                    (Grammar) finObject, (InputTableModel) table.getModel());
            bp.inputField.setEditable(false);
            if (getEnvironment().myTestStrings != null
                    && getEnvironment().myTestStrings.size() > 0) {
    ...
    private int getMachineIndexBySelectedRow(JTable table) {
        ...
        String machineFileName = model.getValueAt(row, 0);
    ...
    protected void updateView(String machineFileName, String input, JTableExtender table) {
        ...
        if (current instanceof Automaton && ((InputTableModel) table.getModel()).isMultiple) {
            int spot = getMachineIndexBySelectedRow(table);
            Automaton cur = null;
            if (spot != -1) {
            ...
            JSplitPane split = SplitPaneFactory.createSplit(getEnvironment(), true, 0.5, newAP,
                    myPanel);
            MultiplePane mp = new MultiplePane(split);
            ...
            BruteParsePane bp = new BruteParsePane((GrammarEnvironment) getEnvironment(), cur,
                    null);
            int column = 1;
            if (spot == -1) {
                column = 0;
            ...
            bp.inputField
                    .setText((String) table.getModel().getValueAt(table.getSelectedRow(), column));
            // bp.inputField.setEnabled(false);
            bp.inputField.setEditable(false);
            JSplitPane split = SplitPaneFactory.createSplit(getEnvironment(), true, 0.5, bp,
                    myPanel);
            MultiplePane mp = new MultiplePane(split);
            getEnvironment().add(mp, getComponentTitle(), new CriticalTag() {
            });
