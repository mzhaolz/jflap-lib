...
public class LRParseTable extends AbstractTableModel implements Serializable, Cloneable {
  ...
  public LRParseTable(Grammar grammar, FiniteStateAutomaton fsa) {
    ...
    for (int i = 0; i < variables.length; i++)
      symbolsToColumn.put(variables[i], new Integer(i + 1 + terminals.length));
    entries = new String[fsa.getStates().length][terminals.length + variables.length + 1];
    for (int i = 0; i < entries.length; i++)
      for (int j = 0; j < entries[i].length; j++) entries[i][j] = j == 0 ? Integer.toString(i) : "";
  }
  ...
  public LRParseTable(LRParseTable table) {
    ...
    for (int i = 0; i < entries.length; i++)
      for (int j = 0; j < entries[i].length; j++) entries[i][j] = table.entries[i][j];
  ...
  private String parseValue(String value, int column) {
    ...
    switch (value.charAt(0)) {
      ...
      case 'r':
        ...
        int startDigits = 1;
        while (!Character.isDigit(value.charAt(startDigits))) startDigits++;
        try {
  ...
  public String getColumnName(int column) {
    if (column == 0) return " ";
    if (column > terminals.length) return variables[column - 1 - terminals.length];
