...
public class EnvironmentFrame extends JFrame {
  ...
  public String getDescription() {
    ...
    else if (environment.myObjects != null
        && environment.getActive() != null
  ...
  public boolean save(boolean saveAs) {
    ...
    while (badname || file == null) {
      if (!badname) {
        ...
        if (file != null) {
          ...
          file =
              new File(
                  Universe.CHOOSER.getCurrentDirectory(), codec.proposeFilename(filename, object));
          // Check for the existing file.
        } else {
          JOptionPane.showMessageDialog(
              null,
              "JFLAP could not determine the selected file name.  Try again.",
              "Error",
              JOptionPane.ERROR_MESSAGE);
          file = null;
          continue;
    ...
    try {
      ...
      if (!blockEdit) environment.setFile(file);
