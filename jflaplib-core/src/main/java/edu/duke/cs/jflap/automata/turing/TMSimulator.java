...
public class TMSimulator extends AutomatonSimulator {
  ...
  public List<Configuration> getInitialConfigurations(String input) {
    ...
    for (int i = 0; i < tapes; i++) inputs.add(input);
  ...
  public List<Configuration> getInitialConfigurations(List<String> inputs) {
    ...
    for (int i = 0; i < tapes.size(); i++) tapes.add(new Tape(inputs.get(i)));
  ...
  private boolean matches(List<Tape> list, TMTransition tmt) {
    ...
    if (list.size() > 1) {
      for (int i = 0; i < list.size(); i++) {
        ...
        // would like to get priorities right for each tape, because the
  ...
  public List<Configuration> stepConfiguration(Configuration config) { // one
    ...
    if (success) { // if variables are used then they will be common to all
      ...
      if (configuration.getTapes().size() > 1) {
          ...
          configuration
              ...
              .writeChar(
                  tmt.getWrite(k).charAt(0) == '~'
                      ? configuration.getTapes().get(k).readChar()
                      : tmt.getWrite(k).charAt(0));
        ...
        configuration
            ...
            .writeChar(
                tmt.getWrite(0).charAt(0) == '~'
                    ? configuration.getTapes().get(0).readChar()
        ...
        list.add(
            new TMConfiguration(
                tmt.getToState(), null, configuration.getTapes(), myFilters)); // no going back
        // - we are in a
        // deterministic
        // world. If you
        // freeze, then
