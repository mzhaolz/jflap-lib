...
public class TMSimulator extends AutomatonSimulator {
    ...
    public TMSimulator(Automaton automaton) {
        ...
        List<AcceptanceFilter> tlist = new ArrayList<>();
        ...
        if (success) { // if variables are used then they will be common to all
            ...
            if (configuration.getTapes().size() > 1) {
                for (int k = 0; k < configuration.getTapes().size(); k++) {
                    configuration.getTapes().get(k)
                            .writeChar(tmt.getWrite(k).charAt(0) == '~'
                ...
                configuration.getTapes().get(0)
                        .writeChar(tmt.getWrite(0).charAt(0) == '~'
    ...
    private Map<String, String> varToChar = new HashMap<>();
