...
public class ConfigurationButton extends JToggleButton {
  ...
  public ConfigurationButton(Configuration config, int state) {
    ...
    this.setPreferredSize(new Dimension(0, PADDING * 2 + icon.getIconHeight()));
  ...
  public void setState(int state) {
    ...
    if (config.isAccept()) state = ACCEPT;
    if (state < 0 || state >= TEXT.length) state = NORMAL;
    // setText(TEXT[state]);
  ...
  public void paintComponent(Graphics g) {
    ...
    icon.paintConfiguration(
        this, (Graphics2D) g, getWidth() - PADDING * 2, getHeight() - PADDING * 2);
    g.translate(-PADDING, -PADDING);
  }
