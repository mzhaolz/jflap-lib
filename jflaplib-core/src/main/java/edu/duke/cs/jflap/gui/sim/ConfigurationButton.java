...
public class ConfigurationButton extends JToggleButton {
	 ...
	 */
	public static final List<Color> STATE_COLOR = ImmutableList.of(new Color(0, 0, 0, 0), new Color(0, 150, 0, 80),
			new Color(255, 0, 0, 80), new Color(100, 100, 255, 80), new Color(255, 255, 0, 80));
	 ...
	 */
	public ConfigurationButton(final Configuration config) {
		this(config, NORMAL);
	}
	 ...
	 */
	public ConfigurationButton(final Configuration config, final int state) {
		super();
		this.config = config;
		...
		setPreferredSize(new Dimension(0, PADDING * 2 + icon.getIconHeight()));
	...
	public void paintComponent(final Graphics g) {
		...
		g.setColor(STATE_COLOR.get(state));
		g.fillRect(0, 0, getWidth(), getHeight());
		g.translate(PADDING, PADDING);

		icon.paintConfiguration(this, (Graphics2D) g, getWidth() - PADDING * 2, getHeight() - PADDING * 2);
		g.translate(-PADDING, -PADDING);
	}
