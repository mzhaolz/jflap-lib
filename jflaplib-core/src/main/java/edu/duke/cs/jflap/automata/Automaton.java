...
public class Automaton implements Serializable, Cloneable {
  ...
  public Object clone() {
    ...
    while (it.hasNext()) {
      ...
      for (int i = 0; i < ts.length; i++) {
        State to = map.get(ts[i].getToState());
        Transition toBeAdded =
            (Transition)
                ts[i]
                    .clone(); //call clone instead of copy so that the gui stuff can get appropriately updated
  ...
  public void removeState(State state) {
    ...
    for (int i = 0; i < t.length; i++) removeTransition(t[i]);
  ...
  public String toString() {
    ...
    for (int s = 0; s < states.length; s++) {
      ...
      if (isFinalState(states[s])) buffer.append(" **FINAL**");
      buffer.append('\n');
