...
/**
 * This class generates {@link edu.duke.cs.jflap.grammar.parse.LRParseTable}s.
 * The intention is that this shall be used once the item goto graph is
 * generated.
 *
...
public abstract class LRParseTableGenerator {
     ...
     */
    public static LRParseTable generate(Grammar grammar, FiniteStateAutomaton gotoGraph,
        ...
        for (int i = 0; i < ts.length; i++) {
            ...
            if (grammar.isVariable(t.getLabel())) {
                // Is a move.
                pt.appendValueAt("" + t.getToState().getID(), t.getFromState().getID(),
                        t.getLabel());
            } else {
                // Is a shift.
                pt.appendValueAt("s" + t.getToState().getID(), t.getFromState().getID(),
                        t.getLabel());
            }
        }
        ...
        for (int i = 0; i < finals.length; i++) {
            ...
            while (it.hasNext()) {
                ...
                if (p.getRHS().endsWith("" + Operations.ITEM_POSITION)) {
                    Production p2 = new Production(p.getLHS(),
                            p.getRHS().substring(0, p.getRHS().length() - 1));
                    int j = 0;
