...
public class UserControlParsePane extends BruteParsePane {
  ...
  protected void initView() {
    ...
    bottomSplit =
        SplitPaneFactory.createSplit(environment, true, 0.3, grammarTable, treeDerivationPane);
    ...
    grammarTable.addKeyListener(
        new KeyListener() {
          ...
          public void keyReleased(KeyEvent e) {
            ...
            if (mySelectedProductionIndex > -1
                && mySelectedProductionIndex < grammarTable.getRowCount() - 1
                && myTarget != null) stepAction.setEnabled(true);
            else stepAction.setEnabled(false);
          }

          public void keyTyped(KeyEvent e) {}
        });

    grammarTable.addMouseListener(
        new MouseListener() {
  ...
  protected void previous() {
    ...
    if (previous.getProductions().length > 0)
      statusDisplay.setText(
          "Derived current Strings using " + previous.getProductions()[0] + " production");
    else statusDisplay.setText("");
    myPreviousCount++;
  ...
  public boolean step() {
    ...
    if (count > 0) {
      if (count > 1) {
        ...
        if (myStringJList.getSelectedIndex() < 0) {
          ...
          for (int i = 0; i < tempIndices.length; i++) {
            if (myJListModel.getElementAt(tempIndices[i]).equals(lhs)) tempCount++;
          }
      ...
      JOptionPane.showMessageDialog(
          this,
          "Previous Derivation does not support this Production",
          "Bad Input",
          JOptionPane.ERROR_MESSAGE);
      stepAction.setEnabled(false);
      return false;
  ...
  private void paintTree() {
    ...
    if (answer.getProductions().length > 0)
      statusDisplay.setText(
          "Derived current Strings using " + answer.getProductions()[0] + " production");
    else statusDisplay.setText("");
