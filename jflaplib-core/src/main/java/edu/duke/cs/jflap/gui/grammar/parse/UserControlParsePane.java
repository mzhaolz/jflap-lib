...
public class UserControlParsePane extends BruteParsePane {
    ...
    protected void previous() {
        ...
        if (previous.getProductions().length > 0)
            statusDisplay.setText("Derived current Strings using " + previous.getProductions()[0]
                    + " production");
    ...
    public boolean step() {
        ...
        if (count > 0) {
            if (count > 1) {
                ...
                if (myStringJList.getSelectedIndex() < 0) {
                    ...
                    for (int i = 0; i < length; i++) {
                        temp = temp
                                + myJListModel.getElementAt(myStringJList.getSelectedIndex() + i);
                    }
            ...
            JOptionPane.showMessageDialog(this,
                    "Previous Derivation does not support this Production", "Bad Input",
