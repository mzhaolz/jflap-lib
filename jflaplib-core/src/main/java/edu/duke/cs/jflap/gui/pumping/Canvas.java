...
public class Canvas extends JPanel {
  ...
  public Text addText(String s) {
    ...
    if (myInitialText.isEmpty()) {
      ...
      p =
          new Point2D.Double(
              q.getX() + t.getWidth(this.getGraphics()) + Text.SPACE.getWidth(this.getGraphics()),
              q.getY());
    }
    Text u = new Text(s, p);
    if (u.toString().length() == 0) u = new Text(Text.SPACE.toString(), p);
    myInitialText.add(u);
    return u;
  }
  ...
  public void moveText(List<Integer> arrayList) {
    ...
    for (int i = 0; i < myInitialText.size(); i++) {
      ...
      for (int j = 0; j < arrayList.get(i); j++) {
        ...
        else p = new Point2D.Double(SECOND_ROW.x + distance, SECOND_ROW.y);
        distance += s.getWidth(this.getGraphics()) + 2; // + 2 is a
        // little extra
        // space between
        // words
  ...
  public void paintComponent(Graphics pen) {
    ...
    if (wait) return;
    else myRestartButton.setEnabled(true);
    ...
    if (!myMovingText.isEmpty()) {
      ...
      if (m.move()) {
        ...
        if (myMovingText.isEmpty()) myStepButton.setEnabled(false);
        return;
      }
      m.paint(pen);
    }
