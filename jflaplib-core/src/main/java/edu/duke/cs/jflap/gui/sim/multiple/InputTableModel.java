...
public class InputTableModel extends GrowableTableModel<Object> {
  ...
  public String getColumnName(int column) {
    ...
    if (getInputCount() == 1) return word;
  ...
  public String[][] getInputs() {
    ...
    for (int r = 0; r < inputs.length; r++) {
      ...
      for (int c = 0; c < inputs[r].length; c++) inputs[r][c] = (String) getValueAt(r, c + begin);
    }
    return inputs;
  }
  ...
  public static InputTableModel getModel(Automaton automaton, boolean multipleFile) {
    ...
    if (model != null && (model.isMultiple == multipleFile)) {
      ...
      for (int i = 0; i < (model.getRowCount() - 1); i++) model.setResult(i, "", null, null, 0);
    } else {
      int add = 0;
  ...
  public void setResult(
    ...
    if (config instanceof TMConfiguration && config != null) {
      ...
      for (int i = 0; (halfway + i) < this.columns; i++) setValueAt("", row, halfway + i);
    ...
    if (config == null) rowToAssociatedConfiguration.remove(new Integer(row));
    else rowToAssociatedConfiguration.put(new Integer(row), config);
  }
