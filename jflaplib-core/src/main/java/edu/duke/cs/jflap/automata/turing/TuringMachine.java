...
public class TuringMachine extends Automaton {
  ...
  public Object clone() {
    ...
    for (Object o : states) {
      ...
      TMState ntms =
          new TMState(
              tms); //I could write a clone for that TM too, I suppose, but there's nothing wrong with a nice C++ style copy constructor
      ntms.setAutomaton(
          a); //recognize thine new master, after the convenience of the copy constructor, lest there be great many bugs.
  ...
  public static void become(TuringMachine dest, TuringMachine src) {
    ...
    for (Object o : src.states) {
      ...
      TMState nstate =
          new TMState(
              state.getID(),
              new Point(state.getPoint()),
              dest); //this time we're not using copy constructor
  ...
  private TMState parent =
      null; //not going to force it with compiler, just make sure you set it WHERE it MATTERS
}
