...
public class W1CW2CW3CW4 extends ContextFreePumpingLemma {
    ...
    public String getHTMLTitle() {
        return "<i>w<sub>1</sub>cw<sub>2</sub>cw<sub>3</sub>cw<sub>4</sub></i>, : "
                ...
                + "<i>w<sub>3</sub></i> = <i>w<sub>4</sub></i>, " + "<i>w<sub>i</sub></i> "
                + ELEMENT_OF + " " + AB_STAR + ", |<i>w<sub>i</sub></i>| > 0";
    }
    ...
    protected void chooseW() {
        w = pumpString("a", m) + pumpString("b", m) + 'c' + pumpString("a", m) + pumpString("b", m)
                + "cacb";
    }

    @Override
    public void chooseDecomposition() {
        List<String> wArray = getWs(w);
        List<Integer> decomp = checkIfPossibility(wArray.get(0), wArray.get(1), 0);
        if (decomp != null) {
            setDecomposition(decomp);
            return;
        }
        decomp = checkIfPossibility(wArray.get(2), wArray.get(3),
                wArray.get(0).length() + wArray.get(1).length() + 2);
        if (decomp != null) {
            setDecomposition(decomp);
            return;
        }
     ...
     */
    private List<Integer> checkIfPossibility(String s0, String s1, int shift) {
        int x;
        String first, last;
        ...
        for (
            ...
            for (int j = 0; j < i; j++) {
                if (s1.charAt(i) == s1.charAt(j)) {
                    first = s1.substring(0, i) + pumpString("" + s1.charAt(i), 2)
                            + s1.substring(i + 1, s1.length());
                    last = s1.substring(0, j) + pumpString("" + s1.charAt(j), 2)
                            + s1.substring(j + 1, s1.length());
                    x = s1.length() - i + j;
