...
public void chooseI() {
  ...
  private List<int> checkIfPossibility(String s0, String s1, int shift) {
    ...
    else if (s0.length() == 1) return new int[] {shift + s0.length() + 1, 1, 0, 0};
    else if (s1.length() == 1 || !s0.equals(s1)) return new int[] {shift, 1, 0, 0};
    else if (m >= s1.length() + 2) return new int[] {shift, 1, s0.length(), 1};
    ...
    for (int i = s1.length() - m + 2; i < s1.length(); i++)
      for (int j = 0; j < i; j++)
        if (s1.charAt(i) == s1.charAt(j)) {
          first =
              s1.substring(0, i)
                  + pumpString("" + s1.charAt(i), 2)
                  + s1.substring(i + 1, s1.length());
          last =
              s1.substring(0, j)
                  + pumpString("" + s1.charAt(j), 2)
                  + s1.substring(j + 1, s1.length());
          x = s1.length() - i + j;
          if (first.equals(last) && x <= m - 2) return new int[] {shift + i, 1, x, 1};
        }
    return null;
  }
...
public boolean isInLang(String s) {
    ...
    for (int i = 0; i < 4; i++)
      if (LemmaMath.otherCharactersFound(w[i], list) || w[i].length() == 0) return false;

    if (w[0].equals(w[1]) || w[2].equals(w[3])) return true;
    return false;
  }
}
