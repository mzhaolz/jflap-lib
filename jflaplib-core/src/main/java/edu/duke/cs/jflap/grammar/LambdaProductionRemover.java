...
public class LambdaProductionRemover {
  ...
  public static boolean isVariableWithLambdaProduction(String variable, Grammar grammar) {
    ...
    for (int k = 0; k < productions.size(); k++) {
      if (ProductionChecker.isLambdaProduction(productions.get(k))) return true;
    }
    return false;
  }
  ...
  public static String getNewVariableWithLambdaProduction(Grammar grammar, Set<String> lambdaSet) {
    ...
    for (int k = 0; k < variables.size(); k++) {
      if (!lambdaSet.contains(variables.get(k))
          && isVariableWithLambdaProduction(variables.get(k), grammar)) {
  ...
  public static boolean isReducableToLambdaProduction(
    ...
    for (int j = 0; j < variables.size(); j++) {
      if (!isInLambdaSet(variables.get(j), lambdaSet)) return false;
    }
    return true;
  }
  ...
  public static String getNewVariableThatBelongsInLambdaSet(
    ...
    for (int k = 0; k < variables.size(); k++) {
      if (!isInLambdaSet(variables.get(k), lambdaSet)
          && belongsInLambdaSet(variables.get(k), grammar, lambdaSet)) return variables.get(k);
    }
    return null;
  }
   ...
   */
  public static List<Production> getProductionsToAddForProduction(
      Production production, Set<String> lambdaSet) {
    // Stupid...
    /*
     ...
     * String[] lambdaVar = (String[]) list
     * combs = getCombinations(lambdaVar); ArrayList productions = new
     * ArrayList(); for(int k = 0; k < combs.size(); k++) { Production p =
    ...
    for (int i = 0; i < rhs.length(); i++) {
      ...
      if (lambdaSet.contains(v)) {
        List<String> s = Collections.nCopies(2 * start.size(), null);
        ...
        for (int j = 0; j < start.size(); j++) start.set(j, start.get(j) + v);
      }
    }
   ...
   *         <CODE>lambdaSet</CODE>.
   */
  public static List<Production> getProductionsToAddToGrammar(
      Grammar grammar, Set<String> lambdaSet) {
    List<Production> list = new ArrayList<>();
    List<Production> productions = grammar.getProductions();
    for (int k = 0; k < productions.size(); k++) {
      List<Production> prods = getProductionsToAddForProduction(productions.get(k), lambdaSet);
  ...
  public static List<Production> getNonLambdaProductions(Grammar grammar) {
    ...
    List<Production> productions = grammar.getProductions();
    for (int k = 0; k < productions.size(); k++) {
      if (!ProductionChecker.isLambdaProduction(productions.get(k))) list.add(productions.get(k));
    }
    return list;
  }
