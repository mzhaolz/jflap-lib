...
public class TMTransducer extends AutomatonTransducer {
  ...
  protected Transition createTransition(
    ...
    while (it.hasNext()) {
      ...
      for (int i = 0; i < nodes.getLength(); i++) {
        ...
        if (tapeString.length() == 0) tapeString = "1"; // Default single tape.
        int tape = 1;
        try {
    ...
    try {
      ...
      TMTransition t = new TMTransition(from, to, readStrings, writeStrings, moveStrings);
  ...
  protected Element createTransitionElement(Document document, Transition transition) {
    ...
    for (int i = 0; i < tm.tapes(); i++) {
      ...
      if (read.equals(TMTransition.BLANK)) read = "";
      if (write.equals(TMTransition.BLANK)) write = "";
  ...
  public Document toDOM(java.io.Serializable structure) {
    ...
    if (tm.tapes() > 1) {
      Element tapesElement = createElement(dom, TURING_TAPES_NAME, null, "" + tm.tapes());
      ...
      else dom.getDocumentElement().insertBefore(tapesElement, list.item(1));
    }
    return dom;
  }
