...
public class JFLAP3Codec extends Codec {
        ...
        for (int s = 0; s < groups.length; s++) {
            for (int g = 0; g < groups[s].length; g++) {
                ...
                try {
                    ...
                    // Create the transition.
                    if (tm.tapes() == 1) {
                        t = new edu.duke.cs.jflap.automata.turing.TMTransition(from, to, group[0],
                                group[2], group[3].toUpperCase());
                    } else {
    ...
    private List<State> readStateCreate(Automaton automaton, BufferedReader reader)
        ...
        List<State> states = Stream.generate(Point::new).limit(numStates)
        ...
        checkArgument(initStateID < 1 || initStateID > states.size(),
     ...
     */
    private String[][][] readTransitionGroups(int groupSize, int idPosition, int numStates,
            BufferedReader reader) throws IOException {
        String[][][] groups = new String[numStates][][];
        for (int s = 0; s < numStates; s++) {
            ParseException p = new ParseException(
                    "Transition line " + (s + 1) + " badly formatted.");
            String[] tokens = reader.readLine().split("\\s+");
    ...
    public static final String FINITE_AUTOMATON_SUFFIX = ".FA", PUSHDOWN_AUTOMATON_SUFFIX = ".PDA",
            TURING_MACHINE_SUFFIX = ".TM", TWO_TAPE_TURING_MACHINE_SUFFIX = ".TTM",
            GRAMMAR_SUFFIX = ".GRM", REGULAR_EXPRESSION_SUFFIX = ".REX";
