...
public class JFLAP3Codec extends Codec {
  ...
  private Automaton readAutomaton(File file) {
    try (BufferedReader reader = new BufferedReader(new FileReader(file))){
      ...
      if (line.equals(PUSHDOWN_AUTOMATON_CODE)) return readPDA(reader);
      if (line.equals(TURING_MACHINE_CODE)) return readTM(reader);
  ...
  private edu.duke.cs.jflap.automata.turing.TuringMachine readTM(BufferedReader reader)
    ...
    try {
      ...
      if (tapes != 1 && tapes != 2) throw new ParseException("May only have 1 or 2 tapes!");
    ...
    for (int s = 0; s < groups.length; s++) {
      for (int g = 0; g < groups[s].length; g++) {
        ...
        try {
          ...
          int[] check = tm.tapes() == 1 ? new int[] {0, 2} : new int[] {0, 2, 4, 5};
          for (int i = 0; i < check.length; i++)
            if (group[check[i]].equals("B")) group[check[i]] = "";
          // Create the transition.
          if (tm.tapes() == 1)
            t =
                new edu.duke.cs.jflap.automata.turing.TMTransition(
                    from, to, group[0], group[2], group[3].toUpperCase());
          else
            t =
                new edu.duke.cs.jflap.automata.turing.TMTransition(
                    from,
  ...
  private State[] readStateCreate(Automaton automaton, BufferedReader reader) throws IOException {
    ...
    if (lineTokens.length == 0) throw new ParseException("Final state list is empty line!");
    try {
  ...
  private String[][][] readTransitionGroups(
      int groupSize, int idPosition, int numStates, BufferedReader reader) throws IOException {
    String[][][] groups = new String[numStates][][];
    for (int s = 0; s < numStates; s++) {
      ParseException p = new ParseException("Transition line " + (s + 1) + " badly formatted.");
      String[] tokens = reader.readLine().split("\\s+");
      if ((tokens.length % groupSize) != 1 || !tokens[tokens.length - 1].equals("EOL")) throw p;
      groups[s] = new String[tokens.length / groupSize][];
      for (int g = 0; g < groups[s].length; g++) {
        groups[s][g] = new String[groupSize];
        for (int i = 0; i < groupSize; i++) groups[s][g][i] = tokens[groupSize * g + i];
        try {
  ...
  private static final String
      FINITE_AUTOMATON_CODE = "One-Way-FSA",
