...
public class AutomatonPane extends JPanel implements Scrollable {
  ...
  public void printComponent(Graphics g) {
    ...
    if (transformNeedsReform) reformTransform(g.getClipBounds());
    g.setColor(java.awt.Color.white);
    g.fillRect(0, 0, g.getClipBounds().width, g.getClipBounds().height);
  ...
  public void reformTransform(Rectangle viewBounds) {
    ...
    if (!adapt) {
      ...
      Point viewportUpperLeft =
          new Point(
              (int) (visible.x - transform.getTranslateX()),
              (int) (visible.y - transform.getTranslateY()));
      // What is the point of the upper left corner of the
      // scrolling component in automaton drawer space?
      Point componentUpperLeft =
          new Point(
              Math.min(bounds.x, Math.min(0, viewportUpperLeft.x)),
      ...
      Dimension newSize =
          new Dimension(
              Math.max(bounds.width + bounds.x, viewportUpperLeft.x + visible.width)
                  - componentUpperLeft.x,
              Math.max(bounds.height + bounds.y, viewportUpperLeft.y + visible.height)
                  - componentUpperLeft.y);
      if (newSize.equals(getPreferredSize())) return;
    ...
    if (aRatio > vRatio) {
      ...
      bounds.setRect(
          bounds.getX() - targetWidth / 2.0,
          bounds.getY(),
          bounds.getWidth() + targetWidth,
          bounds.getHeight());
    }
  ...
  private class Listener extends ComponentAdapter
    ...
    public void componentResized(ComponentEvent e) {
      if (adapt) transformNeedsReform = true;
      repaint();
    }
  }
  ...
  public void fitToBounds(int padding) {
    ...
    for (int i = 0; i < states.length; i++) {
      ...
      p.setLocation(
          (p.getX() - automatonBounds.getX())
              * (viewBounds.getWidth() / automatonBounds.getWidth()),
          (p.getY() - automatonBounds.getY())
  ...
  public Component add(Component c) {
    if (c instanceof JTable) table = (JTable) c;
    return super.add(c);
  }
