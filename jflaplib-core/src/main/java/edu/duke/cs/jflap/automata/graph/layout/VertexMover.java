  ...
  public void layout(Graph<V> graph, Set<V> notMoving) {
    ...
    if (command == HORIZONTAL_CENTER)
      for (int i = 0; i < vertices.size(); i++) {
        ...
        graph.moveVertex(
            vertices.get(i), new Point2D.Double(point.getX(), size.getHeight() - point.getY()));
      }
    else if (command == VERTICAL_CENTER)
      for (int i = 0; i < vertices.size(); i++) {
        point = graph.pointForVertex(vertices.get(i));
        graph.moveVertex(
            vertices.get(i), new Point2D.Double(size.getWidth() - point.getX(), point.getY()));
      }
      ...
      try {
        ...
        String input =
            JOptionPane.showInputDialog(
                "How many degrees to turn the automaton? \n"
                    + "(positive - clockwise, negative - counterclockwise)",
                new String("180"));
        theta = Double.parseDouble(input);
      } catch (NumberFormatException e) {
        JOptionPane.showMessageDialog(
            null, "That is not a valid degree value.", "Invalid Input", JOptionPane.ERROR_MESSAGE);
        return;
