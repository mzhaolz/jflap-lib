...
/**
 * Temporary tester for turing trimming algo TODO: delete this tester class
 *
...
public class TuringTrimmingTester {
	...
	public TuringTrimmingTester(final String filename) {
		int index = 0;
		Scanner sc;
		...
		try {
			...
			while (sc.hasNextLine()) {
				...
				final Production p = new Production(aa[0], aa[1]);
				prods[index] = p;
				index++;
			}
		...
		for (final Production prod : prods) {
			System.out.println(prod.getLHS() + "->" + prod.getRHS());
		}
	}
