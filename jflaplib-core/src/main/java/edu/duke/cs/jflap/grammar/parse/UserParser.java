...
public abstract class UserParser {
    ...
    public int checkValidAndParse(int index) {
        ...
        for (int i = 0; i < myAnswer.getDerivation().length(); i++) {
            ...
            if (myAnswer.getDerivation().substring(i, i + length)
                    .equals(myCurrentProduction.getLHS())) {
                count++;
            }
        }
        return count;
    ...
    private ParseNode getNextResult(String c, int index) {
        ...
        start = index;
        String prepend = c.substring(0, start) + myCurrentProduction.getRHS()
    ...
    private ParseNode getNextSubstitution(String c, List<Integer> tempIndices) {
        ...
        for (int i = 0; i < multipleSubstitutionArray.size(); i++) {
            if (i == 0) {
                ...
                start = multipleSubstitutionArray.get(i)
                        + i * (myCurrentProduction.getRHS().length() - 1);
            }
