...
public class LLParseDerivationController {
    ...
    boolean done() {
        switch (step) {
            ...
            case FOLLOW_SETS:
                ...
                JOptionPane.showMessageDialog(parseTable, "Highlighted sets are incorrect.",
            ...
            case PARSE_TABLE:
                ...
                List<List<String>> diff = pt.getDifferences(targetParseTable);
                if (diff.size() == 0) {
                    return true;
                }
                for (List<String> element : diff) {
                    int row = pt.getRow(element.get(0));
                    int column = pt.getColumn(element.get(1));
     ...
     */
    public void parse() {
        LLParsePane panel = new LLParsePane(environment, grammar, targetParseTable);
