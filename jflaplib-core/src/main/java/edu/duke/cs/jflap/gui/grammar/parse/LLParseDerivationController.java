...
public class LLParseDerivationController {
	 ...
	 */
	public LLParseDerivationController(final Grammar grammar, final GrammarEnvironment environment,
			final FirstFollowTable firstFollow, final LLParseTablePane parseTable, final JLabel directions) {
	...
	public void completeSelected() {
		...
		case FIRST_SETS:
		case FOLLOW_SETS:
			final Map<?, ?> sets = step == FIRST_SETS ? targetFirstSets : targetFollowSets;
			final int col = step == FIRST_SETS ? 1 : 2;
			final FirstFollowModel ffm = firstFollow.getFFModel();
			final int viewCol = firstFollow.convertColumnIndexToView(col);
			for (int i = 0; i < ffm.getColumnCount(); i++) {
				if (!firstFollow.isCellSelected(i, viewCol)) {
					continue;
				}
				// ////System.out.println("Doing row "+i);
				final String var = (String) ffm.getValueAt(i, 0);
			...
			break;
		case PARSE_TABLE:
			final LLParseTable pt = parseTable.getParseTable();
			for (int r = 0; r < pt.getRowCount(); r++) {
				pt.getValueAt(r, 0);
				for (int c = 1; c < pt.getColumnCount(); c++) {
					final int cv = parseTable.convertColumnIndexToView(c);
			...
			break;
		case FINISHED:
			JOptionPane.showMessageDialog(firstFollow, "The parse table is complete.", "Finished",
	...
	boolean done() {
		...
		case FOLLOW_SETS:
			...
			JOptionPane.showMessageDialog(parseTable, "Highlighted sets are incorrect.", "Bad Sets",
					JOptionPane.ERROR_MESSAGE);
		...
		case PARSE_TABLE:
			...
			final List<List<String>> diff = pt.getDifferences(targetParseTable);
			if (diff.size() == 0) {
				return true;
			}
			for (final List<String> element : diff) {
				final int row = pt.getRow(element.get(0));
				final int column = pt.getColumn(element.get(1));
				parseTable.highlight(row, column);
			}
			parseTable.clearSelection();
			JOptionPane.showMessageDialog(firstFollow, "Highlighted cells are incorrect.", "Bad Parse Table",
					JOptionPane.ERROR_MESSAGE);
	...
	public boolean nextStep() {
		...
		case PARSE_TABLE:
			firstFollow.getFFModel().setCanEditFollow(false);
			directions.setText("Fill entries in parse table.  Use ! for a lambda entry.");
			break;
		case FINISHED:
			try {
				parseTable.getCellEditor().stopCellEditing();
	 ...
	 */
	public void parse() {
		final LLParsePane panel = new LLParsePane(environment, grammar, targetParseTable);
