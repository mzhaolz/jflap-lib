      ...
      case VARAIBLE_GRAPH:
        {
          ...
          for (int i = 0; i < s.length; i++)
            if (terminalVariables.contains(s[i].getName())) vdg.addFinalState(s[i]);
          // Cache the transitions we have to add.
          Production[] p = grammar.getProductions();
          String[] variables = (String[]) terminalVariables.toArray(new String[0]);
      ...
      case START_NOT_TERMINAL_DERIVING_ERROR:
        {
          pane.mainLabel.setText(
              "The start variable " + grammar.getStartVariable() + " does not derive terminals.");
          pane.detailLabel.setText(
              "The grammar cannot have rules.  " + "No further action is sensible.");
          pane.doStepAction.setEnabled(false);
          pane.doAllAction.setEnabled(false);
          pane.proceedAction.setEnabled(false);
          pane.exportAction.setEnabled(false);
  ...
  void doStep() {
    switch (step) {
      case DERIVE_TERMINALS:
        ...
        break;
      case VARAIBLE_GRAPH:
        Transition[] t = (Transition[]) vdgTransitions.toArray(new Transition[0]);
      ...
      case DERIVE_TERMINALS:
        ...
        if (derivedTerminalVariables.contains(var)) {
          ...
          pane.detailLabel.setText(
              var
                  + " already selected!  "
                  + (terminalVariables.size() - derivedTerminalVariables.size())
        ...
        if (terminalVariables.contains(var)) {
          ...
          pane.terminalLabel.setText(
              "Variables that predicate terminals: " + derivedTerminalVariables);
          if (derivedTerminalVariables.size() == terminalVariables.size()) nextStep();
          return;
        }
        pane.detailLabel.setText(
            var
