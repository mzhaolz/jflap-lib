...
public class PDANondeterminismDetector extends NondeterminismDetector {
	 ...
	 */
	@Override
	public boolean areNondeterministic(final Transition t1, final Transition t2) {
		final PDATransition transition1 = (PDATransition) t1;
		final PDATransition transition2 = (PDATransition) t2;
		final String input1 = transition1.getInputToRead();
		final String input2 = transition2.getInputToRead();
		final String toPop1 = transition1.getStringToPop();
		final String toPop2 = transition2.getStringToPop();

		if (arePrefixesOfEachOther(input1, input2) && arePrefixesOfEachOther(toPop1, toPop2)) {
			return true;
		} else {
			return false;
		}
	 ...
	 *         string.
	 */
	public boolean arePrefixesOfEachOther(final String s1, final String s2) {
