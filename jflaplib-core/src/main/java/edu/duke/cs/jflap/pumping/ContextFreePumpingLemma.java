...
public abstract class ContextFreePumpingLemma extends PumpingLemma
  ...
  public String getDecompositionAsString() {
    ...
    for (int i = 0; i < s.length; i++)
      if (s[i].length() == 0) s[i] = Universe.curProfile.getEmptyString(); // lower case
    // lambda
  ...
  public int addCase(int[] decomposition, int num) {
     ...
     */
    if (!setDecomposition(decomposition)) return -1;
    /*
     * Check if this case has been done.
     */
    for (int j = 0; j < myDoneCases.size(); j++) {
      if ((myDoneCases.get(j)).isCase(v, y)) return j;
    }
    /*
  ...
  private void chooseDecompositionWithoutCases() {
    ...
    if (decomp[1] == w.length() - counter) {
      ...
      if (decomp[1] > 0) decomp[3] = LemmaMath.fetchRandInt(0, temp);
