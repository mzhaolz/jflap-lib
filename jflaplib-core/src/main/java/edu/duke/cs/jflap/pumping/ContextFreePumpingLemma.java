...
public abstract class ContextFreePumpingLemma extends PumpingLemma implements Serializable, Cloneable {
	...
	/**
	 * Adds the decomposition to the list that the user has done. If the case is
	 * not a legal decomposition, the it returns <code>-1</code>. If the case
	 * has already been done, it returns the index of the case. If it hasn't
	 * been done, it moves the case to the "done" list and returns its position
	 * in the done list.
	 *
	 * @param decomposition
	 *            the decomposition we wish to add
	 * @return <code>-1</code> if it is an illegal decomposition, the index of
	 *         the decomposition if it has already been done, or a number
	...
	public int addCase(final List<Integer> decomposition, final int num) {
		...
		for (int j = 0; j < myDoneCases.size(); j++) {
			if ((myDoneCases.get(j)).isCase(v, y)) {
				return j;
			}
		}
		/*
	...
	private void chooseDecompositionWithoutCases() {
		int temp, counter;
		final Integer[] decomp = new Integer[4];
		counter = 0;
		temp = 0;
	...
	/**
	 * Returns the pumped string <i>uv<sup>i</sup>xy<sup>i</sup>z</i> according
	 * the the decomposition and choice of <i>i</i>.
	 *
	...
	public String getDecompositionAsString() {
		...
		for (int i = 0; i < s.length; i++) {
			if (s[i].length() == 0) {
				s[i] = Universe.curProfile.getEmptyString(); // lower case
				// lambda
			}
		}
	 ...
	 *
	 * @param v
	 *            segment <i>v</i> of the decomposition
	 * @param y
	 *            segment <i>y</i> of the decomposition
	 */
	/*
	 * protected boolean decompMatchesUnusedCase(String v, String y) { Case
	 * cAll, cDone; Iterator iAll, iDone; boolean alreadyDone, toReturn;
	 *
	 * toReturn = false; iAll = myAllCases.iterator(); while (iAll.hasNext()) {
	 * alreadyDone = false; cAll = (Case) iAll.next(); if (cAll.isCase(v, y)) {
	 *
	 * iDone = myDoneCases.iterator(); while (iDone.hasNext()) { cDone = (Case)
	 * iDone.next(); if (cDone.isCase(v, y)) alreadyDone = true; }
	 *
	 * if (!alreadyDone) toReturn = true; } } return toReturn; }
	 */

	/**
	 * For values with cases, chooses a decomposition that corresponds to the
	 * first case that hasn't currently been added to the casePanel
	 */
	/*
	 * protected void chooseDecompositionWithCases() { String v, x, y; int[]
	 * decomp; boolean matchesUnusedCase; int size = w.length();
	 *
	 * for (int a=0; a<size; a++) for (int b=a; b<size; b++) for (int c=b;
	 * c<size; c++) for (int d=c; d<size; d++) { v = getW().substring(a, b); x =
	 * getW().substring(b, c); y = getW().substring(c, d);
	 *
	...
	public boolean setDecomposition(final List<Integer> decomposition) {
		...
		final int uLength = decomposition.get(0);
		final int vLength = decomposition.get(1);
		final int xLength = decomposition.get(2);
		final int yLength = decomposition.get(3);
	...
	/**
	 * Sets <i>i</i> and chooses the decomposition, with the length of each part
	 * of the decomposition in the corresponding space of the input array. That
	 * is, |<i>u</i>| = <code>decomposition[0]</code>, |<i>v</i>| =
	 * <code>decomposition[1]</code>, |<i>x</i>| =
	 * <code>decomposition[2]</code>, |<i>y</i>| =
	 * <code>decomposition[3]</code>.
	 *
	 * @param decomposition
	 *            the array that contains the length of each part of the
	 *            decomposition
