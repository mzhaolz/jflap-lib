...
public class FSALabelHandler {
    ...
    public static void handleLabel(Transition transition, Automaton automaton) {
        ...
        for (int i = 0; i < length; i++) {
            State going = i == length - 1 ? to
                    : automaton.createState(new java.awt.Point(
                            (f.getPoint().x * (length - i - 1) + to.getPoint().x * (i + 1))
                                    / length,
                            (f.getPoint().y * (length - i - 1) + to.getPoint().y * (i + 1))
                                    / length));
            Transition newTrans = new FSATransition(from, going, label.substring(i, i + 1));
    ...
    public static void splitLabel(Transition transition, Automaton automaton) {
        ...
        automaton.removeTransition(trans);
        String label = trans.getLabel();
        for (int i = label.charAt(label.indexOf("[") + 1); i <= label
                .charAt(label.indexOf("[") + 3); i++) {
    ...
    public static FiniteStateAutomaton removeMultipleCharacterLabels(Automaton automaton) {
        ...
        List<Transition> transitions = fsa.getTransitions();
        for (int k = 0; k < transitions.size(); k++) {
            FSATransition transition = (FSATransition) transitions.get(k);
            String label = transition.getLabel();
            if (label.length() > 1) {
