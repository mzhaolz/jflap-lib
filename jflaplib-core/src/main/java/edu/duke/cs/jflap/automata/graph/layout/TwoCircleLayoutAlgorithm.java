...
public class TwoCircleLayoutAlgorithm<V> extends LayoutAlgorithm<V> {
  ...
  protected void assignToCircles() {
    for (int i = 0; i < vertices.size(); i++)
      if (graph.degree(vertices.get(i)) > 2) innerCircle.add(vertices.get(i));
    ...
    do {
      ...
      for (int i = 0; i < outerCircle.size(); i++) {
        ...
        for (int j = 0; j < innerCircle.size(); j++)
          if (graph.hasEdge(outerCircle.get(i), innerCircle.get(j))) count++;
        if (count >= 2) {
  ...
  protected void createOuterCircleChains() {
      ...
      for (int i = 0; i < outerCircle.size(); i++) {
        if (chainIndex[i] == -1) {
          ...
          if (match1 > -1 && match2 == -1) {
            ...
            if (outerCircleChains[match1].size() < outerCircleChains[match2].size()) min = match1;
            else min = match2;
    ...
    for (int i = 0; i < outerCircleChains.length; i++)
      if (outerCircleChains[min].getVertices().size() > outerCircleChains[i].getVertices().size())
        min = i;
    for (int i = 0; i < outerCircle.size(); i++)
      if (chainIndex[i] == -1) outerCircleChains[min].addVertex(outerCircle.get(i));
  }
  ...
  protected void shuffleOuterChains() {
    ...
    for (int i = 0; i < outerCircleChains.length; i++) {
      ...
      else nextChain = outerCircleChains[0];
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < vertices.length; i++) {
      ...
      if (i > 0) graph.addEdge(vertices[i], vertices[i - 1]);
      // if (i>1 && i<5)
    ...
    for (int i = 0; i < vertices.length; i++)
      for (int j = 0; j < vertices.length; j++)
        if (graph.hasEdge(vertices[i], vertices[j])) {
          p.getGraphics()
              .drawLine(
                  (int) (graph.pointForVertex(vertices[i]).getX()),
