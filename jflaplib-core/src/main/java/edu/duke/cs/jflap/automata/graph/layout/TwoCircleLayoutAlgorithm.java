...
public class TwoCircleLayoutAlgorithm<V> extends LayoutAlgorithm<V> {
    ...
    protected void createOuterCircleChains() {
            ...
            for (int i = 0; i < outerCircle.size(); i++) {
                if (chainIndex[i] == -1) {
                    ...
                    for (int j = 0; j < outerCircleChains.length; j++) {
                        if (outerCircleChains[j].isEdgeToChainMember(outerCircle.get(i))
                                && chainIndex[i] == -1) {
                            if (match1 == -1) {
                                match1 = j;
                            } else {
        ...
        min = 0;
        for (int i = 0; i < outerCircleChains.length; i++) {
            if (outerCircleChains[min].getVertices().size() > outerCircleChains[i].getVertices()
                    .size()) {
                min = i;
            }
        }
    ...
    public static void main(String[] args) {
        ...
        for (String vertice : vertices) {
            Point2D p2d = graph.pointForVertex(vertice);
            ...
            for (String vertice2 : vertices) {
                if (graph.hasEdge(vertice, vertice2)) {
                    p.getGraphics().drawLine((int) (graph.pointForVertex(vertice).getX()),
                            (int) (graph.pointForVertex(vertice).getY()),
                            (int) (graph.pointForVertex(vertice2).getX()),
