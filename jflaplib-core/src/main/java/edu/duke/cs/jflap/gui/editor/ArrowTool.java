...
public class ArrowTool extends Tool {
    ...
    public void mousePressed(MouseEvent event) {
        ...
        List<Transition> trans = getAutomaton().getTransitions();
    ...
    public void mouseDragged(MouseEvent event) {
        ...
        if (lastClickedState != null) {
            ...
            List<State> states = getView().getDrawer().getAutomaton().getStates();
            ...
            if (f != t) {
                ...
                // angle-Math.PI*.166);
                List<Transition> trans = getAutomaton().getTransitionsFromStateToState(f, t);
                for (Transition tran : trans) {
                    CurvedArrow arrow = getView().getDrawer().transitionToArrowMap.get(tran);
        ...
        if (selectedTransition != null) { // simply set ...but we need to get
            ...
            if (transitionInFlux
                    || Math.sqrt((control.getX() - myClickP.x) * (control.getX() - myClickP.x)
    ...
    protected class StateMenu extends JPopupMenu implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (item == makeFinal) {
                ...
                EditBlockPane editor = new EditBlockPane(((TMState) state).getInnerTM()); // give
                // it
                // a
                // Turing
                // Machine
                ...
                TMState buffer = ((TuringMachine) getAutomaton())
                ...
                // with
                // Turing.
                buffer.setInnerTM((TuringMachine) ((TMState) state).getInnerTM().clone()); // all
                // states
                // have
                // an
                // inner
        ...
        private void replaceCharactersInBlock(TMState start, String toReplace, String replaceWith) { // this
            // shall
            // be
            // a
            // recursive
            ...
            List<Transition> trans = tm.getTransitions();

            for (Transition tran : trans) {
                TMTransition tmTrans = (TMTransition) tran;
        ...
        private JMenuItem changeLabel, deleteLabel, deleteAllLabels, editBlock, copyBlock,
                replaceSymbol, setName;
    }
