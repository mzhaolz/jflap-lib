...
public class ArrowTool extends Tool {
  ...
  public void mousePressed(MouseEvent event) {
    if (getDrawer().getAutomaton().getEnvironmentFrame() != null)
      ((AutomatonEnvironment) getDrawer().getAutomaton().getEnvironmentFrame().getEnvironment())
          .saveStatus();
    else EDebug.print("I cannot preserve what you ask");
  ...
  public void mouseDragged(MouseEvent event) {
    ...
    if (lastClickedState != null) {
      ...
      bounds =
          new Rectangle(
              leftX,
              topY,
              Math.abs(nowX - initialPointClick.x),
    ...
    if (selectedTransition
      ...
      if (transitionInFlux
          || Math.sqrt(
                  (control.getX() - myClickP.x) * (control.getX() - myClickP.x)
                      + (control.getY() - myClickP.y) * (control.getY() - myClickP.y))
              < 15) {
  ...
  protected class StateMenu extends JPopupMenu implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (item == makeFinal) {
        if (item.isSelected()) getAutomaton().addFinalState(state);
        else getAutomaton().removeFinalState(state);
        ...
        for (int i = 0; i < states.length; i++) states[i].setLabel(null);
          ...
          == editBlock) { //this implies that this was a TMState to begin with, because only TM states would have this menu option
        ...
        EditBlockPane editor =
            new EditBlockPane(
                ((TMState) state)
                    .getInnerTM()); //give it a Turing Machine //just edit the Automaton directly; there is no need for a repaint either, because the other guy does not paint it
        ...
        TMState buffer =
            ((TuringMachine) getAutomaton())
                .createTMState(
                    new Point(
                        state.getPoint().x + 4,
                        state.getPoint()
                            .y)); //again, we assume that the cast will work, since copyBlock hould never be there except with Turing.
        buffer.setInnerTM(
            (TuringMachine)
                ((TMState) state)
                    .getInnerTM()
                    .clone()); //all states have an inner TM, although this inner TM might have zero states within it, in which case it acts as a simple state.
    ...
    private void replaceCharactersInBlock(
        TMState start,
        String toReplace,
        String
            replaceWith) { //this shall be a recursive method, replacing the inside and then the out
