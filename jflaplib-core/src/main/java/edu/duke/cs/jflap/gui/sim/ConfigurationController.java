...
public class ConfigurationController implements ConfigurationSelectionListener {
  ...
  public ConfigurationController(
    ...
    this.originalConfigurations = (Configuration[]) configurations.getConfigurations();
  ...
  public void step(boolean blockStep) {
    ...
    while (it.hasNext()) {
      ...
      if (reject.contains(config)) configurations.setReject(config);
    }
    // What the devil do I have to do to get it to repaint?
    configurations.validate();
    configurations.repaint();
    ...
    try {
      ...
      // my government...
      JSplitPane split =
          (JSplitPane) configurations.getParent().getParent().getParent().getParent();
  ...
  public void focus() {
    ...
    if (configs.length == 0) {
      ...
      JOptionPane.showMessageDialog(
          configurations,
          FOCUS_CONFIGURATION_ERROR,
          FOCUS_CONFIGURATION_ERROR_TITLE,
          JOptionPane.ERROR_MESSAGE);
      return;
    }
  ...
  private boolean setFocusIfNeeded(Configuration current, boolean foundFocused) {
    ...
    if (parentConfig == null) return foundFocused;
  ...
  public void trace() {
    ...
    for (int i = 0; i < configs.length; i++) {
      TraceWindow window = (TraceWindow) configurationToTraceWindow.get(configs[i]);
      if (window == null) {
        configurationToTraceWindow.put(configs[i], new TraceWindow(configs[i]));
      } else {
  ...
  /** The error message displayed when there is no config selected. */
  private static final String FOCUS_CONFIGURATION_ERROR =
      "JFLAP can only focus on one configuration at a time!";
