...
public class NFAToDFA {
  ...
  public boolean containSameStates(State[] states1, State[] states2) {
    ...
    for (int k = 0; k < states1.length; k++) {
      ...
      //				return false;
      if (states1[k] != states2[k]) return false;
    }
    return true;
  }
  ...
  public List<State> expandState(State state, Automaton nfa, Automaton dfa) {
    ...
    for (int k = 0; k < alphabet.length; k++) {
       ...
       * state.
       */
      State[] states = getStatesOnTerminal(alphabet[k], getStatesForState(state, nfa), nfa);
      ...
      if (states.length > 0) {
         ...
         */
        Transition transition = new FSATransition(state, toState, alphabet[k]);
