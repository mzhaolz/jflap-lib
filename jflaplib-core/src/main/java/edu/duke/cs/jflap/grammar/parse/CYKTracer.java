...
public class CYKTracer {
  ...
  private void initializeLambdaStepMap() {
    ...
    if (lambdaDerivers.size() > 0) {
      ...
      HashMap<String, ArrayList<Production>> indirectLambdaProductions =
  ...
  private void intializeUnitStepMap(Grammar g) {
    if (UnitProductionRemover.getUnitProductions(g).length > 0) {
      ...
      for (int i = 0; i < productionsToAdd.size(); i++) {
        ...
        if (removedUnitProductions.get(var1) == null) reportError();
        else {
  ...
  public void traceBack() {
    ...
    if (myAnswer.size() == 0) myAnswer.addAll(myTrace);
  ...
  private void backTrackToLambda() {
    if (myLambdaStepMap == null) return;
    int index = 0;
    while (index < myAnswer.size()) {
