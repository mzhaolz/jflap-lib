...
public class CYKTracer {
    ...
    private void initializeLambdaStepMap() {
        ...
        if (lambdaDerivers.size() > 0) {
            ...
            for (int i = 0; i < p.length; i++) {
                Production[] p2 = LambdaProductionRemover.getProductionsToAddForProduction(p[i],
                        lambdaDerivers);
                ...
                for (int j = 0; j < p2.length; j++) {
                    ...
                    if (!p2[j].equals(p[i])) {
                        temp.add(p[i]);
                        ArrayList<String> variables = getDifferentVariable(p[i].getRHS(),
                                p2[j].getRHS());
    ...
    private int[] searchForRest(ArrayList<Production> list, Production p, int[] visited) {
        ...
        int count = 0;
        for (int i = 0; i < myTrace.size(); i++) {
            if (list.contains(myTrace.get(i)) && visited[i] == 0
