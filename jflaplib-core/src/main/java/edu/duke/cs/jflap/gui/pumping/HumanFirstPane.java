 ...
 * <code>PumpingLemmaInputPane</code> wherein the user makes the first move. The
 * user will choose the <i>m</i> and decomposition values, while the computer
 * will generate the <i>w</i> and <i>i</i> values based on the user's input. The
 * decomp panel has a network of sliders and displays allowing for the user to
...
public abstract class HumanFirstPane extends PumpingLemmaInputPane {
	...
	public HumanFirstPane(final PumpingLemma l, final String title) {
		super(l, title);
	 ...
	 *
	 * @param p
	 *            the current decomposition panel
	 */
	...
	protected String addMGameFeatures() {
		...
		((JTextField) myMDisplay).addActionListener(ev -> {
			try {
				reset();
				final int m = Integer.parseInt(ev.getActionCommand());
				...
				final String error = "Please enter a positive integer in range [" + myLemma.getRange().get(0) + ", "
						+ myLemma.getRange().get(1) + "] for best results.";
	...
	@Override
	protected String addTopGameFeatures(final JButton b) {
	...
	public void displayEnd() {
		final String s = myLemma.createPumpedString();
		myPumpedStringDisplay.setText(s);
		if (myLemma.isInLang(s)) {
			myLastWord.setText(createXYZ() + " = " + PumpingLemmaInputPane.toHTMLString(s) + " = " + s
					+ " is in the language.  YOU WIN!");
		} else {
			myLastWord.setText(createXYZ() + " = " + PumpingLemmaInputPane.toHTMLString(s) + " = " + s
					+ " is NOT in the language.  Please try again.");
		}
	}

	/**
	 * Initializes and returns a panel that allows the user to choose the
	 * decomposition of <i>w</i> into <i>xyz</i> with sliders. The panel also
	 * contains a single-row table in which each cell contains one character of
	 * <i>w</i> such that each column corresponds to one unit in the sliders.
	 * This is done to aid the visualization process.
	 *
	 * @return a panel that takes the decomposition from the user
	 */
	@Override
	protected JPanel initDecompPanel() {
		final JPanel n = new JPanel(new BorderLayout());
		...
		final JPanel m = new JPanel(); // I'm really running out of letters here
		...
		myDecompButton.addActionListener(ev -> {
			...
			myCanvas.stop();
			/*
			 * If there is only one case and myCases will not have been
			 * initialized.
			 */
			if (myCases != null) {
	...
	/**
	 * Refreshes the various fields accordingly after any action is taken when
	 * adjusting SliderPanels.
	 */
	protected abstract void refresh();

	/**
	 * Calculates <i>i</i> and the associated pumped string based on the user's
	 * decomposition.
	 */
	protected abstract void setI();

	/**
	 * Updates the individual cells in the single-row table such that each show
	 * a character of the string. It resizes the table depending on how many
