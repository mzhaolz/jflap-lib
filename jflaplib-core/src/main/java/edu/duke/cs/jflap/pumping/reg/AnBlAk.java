...
public class AnBlAk extends RegularPumpingLemma {
  ...
  public void setDescription() {
    ...
    explanation =
        "For any <i>m</i> value "
            + GREATER_OR_EQ
            + " 4, a possible value for <i>w</i> is \"a<sup>6</sup>"
            + "b<sup><i>m</i></sup>a<sup><i>m</i></sup>\".  The <i>y</i> value thus would be a combination of \"a\"s "
            + "and \"b\"s, in that order.  If <i>i</i> = 0, either n "
            + LESS_OR_EQ
  ...
  public boolean isInLang(String s) {
    ...
    String ba2 = s.substring(s.indexOf('b')); // deletes the a^n portion of
    // the string
    a = s.length() - ba2.length();
    a2 = LemmaMath.countInstances(ba2, 'a');
    if (a <= 5 || b <= 3 || a2 > b) return false;
    return true;
  }
}
