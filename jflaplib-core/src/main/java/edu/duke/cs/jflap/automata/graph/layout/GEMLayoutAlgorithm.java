        ...
        for (int i = 0; i < Rmax && Tglobal > Tmin; i++) {
            ...
            double[] p = new double[] {
                            ...
                            (c[1] / graph.numberOfVertices() - point.getY())
                                    * GRAVITATIONAL_CONSTANT * Theta }; // Attraction
            // to
            // BC.
            // Random disturbance.
            ...
            for (int j = 0; j < vArray.size(); j++) {
                ...
                double[] delta = new double[] { point.getX() - otherPoint.getX(),
            ...
            if (p[0] != 0.0 || p[1] != 0.0) {
                ...
                // update the position!
                graph.moveVertex(vertex,
                        new Point2D.Double(point.getX() + p[0], point.getY() + p[1]));
