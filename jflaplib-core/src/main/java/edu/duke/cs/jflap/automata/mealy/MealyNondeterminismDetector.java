...
/**
 * The <code>MealyNondeterminismDetector</code> can be used to find all the
 * nondeterministic states in a Mealy or Moore machine (all states with equal
 * outward transitions).
 *
 * @author Jinghui Lim
 *
...
public class MealyNondeterminismDetector extends NondeterminismDetector {
	 ...
	 * equivalent), or if they introduce nondeterminism (for example, the label
	 * of one could be a prefix of the label of the other).
	 *
	 * @param t1
	 *            a transition
	 * @param t2
	...
	public boolean areNondeterministic(final Transition t1, final Transition t2) {
		final MealyTransition transition1 = (MealyTransition) t1;
		final MealyTransition transition2 = (MealyTransition) t2;
