...
public class AkBnCnDj extends ContextFreePumpingLemma {
  ...
  public void setDescription() {
    ...
    explanation =
        "Because this is a context-free language, a valid decomposition exists.  For all m "
            ...
            + "\"d\".  If <i>n</i> = 0 and only one of <i>k</i> or <i>j</i> "
            + GREATER_OR_EQ
  ...
  public void chooseDecomposition() {
    ...
    if (b > -1 && c > -1) setDecomposition(new int[] {c - 1, 1, 0, 1});
    else if (a > -1 && d > -1) setDecomposition(new int[] {d - 1, 1, 0, 1});
  ...
  public boolean isInLang(String s) {
    ...
    j = 0;
    while (i < s.length())
      if (s.charAt(i) != list[j]) j++;
      else {
        sections[j]++;
        i++;
      }

    if (sections[1] == sections[2] && sections[0] != sections[3]) return true;
    return false;
  }
}
