...
public class LSystemDisplay extends LSystemAction {
	 ...
	 */
	public LSystemDisplay(final LSystemEnvironment environment) {
	...
	public void actionPerformed(final ActionEvent e) {
		final LSystem lsystem = getEnvironment().getLSystem();

		if (lsystem.getAxiom().size() == 0) {
			JOptionPane.showMessageDialog(getEnvironment(), "The axiom must have one or more symbols.",
					"Nonempty Axiom Required", JOptionPane.ERROR_MESSAGE);
			return;
		}

		try {
			final DisplayPane pane = new DisplayPane(lsystem);
			...
			JOptionPane.showMessageDialog(getEnvironment(),
					"Sorry, but this uses features requiring Java 1.4 or later!", "JVM too primitive",
