...
public abstract class PumpingLemmaInputPane extends JPanel {
  ...
  public PumpingLemmaInputPane(PumpingLemma l, String title) {
    ...
    JEditorPane ep =
        new JEditorPane("text/html", "<html><body align=center><b>" + title + "</b></body></html>");
  ...
  private void initLeftPanel() {
    ...
    for (int i = 0; i < stageMessages.length; i++) stageMessages[i] = new JLabel();
    ...
    for (int i = 0; i < stages.length; i++) leftPanel.add(stages[i]);
    if (this instanceof HumanFirstPane) stages[2].setVisible(false);
  ...
  protected void setVisibilityStages(int minStage, boolean visibility) {
    for (int i = minStage; i < stages.length; i++) stages[i].setVisible(visibility);
  }
  ...
  protected void resetMessages() {
    for (int i = 0; i < stageMessages.length; i++) stageMessages[i].setText("");
  }
  ...
  protected void reset() {
    ...
    if (this instanceof HumanFirstPane) setVisibilityStages(2, false);
    else setVisibilityStages(3, false);
    leftPanel.revalidate();
  }
  ...
  public void updateTopPane(boolean printExplanation) {
    ...
    if (printExplanation) {
      ...
      else result = "<b>Unfortunately no valid partition of <i>w</i> exists.</b><br>";
      if (myLemma.getAttempts().size() > 0)
        output = output + result + myLemma.getExplanation() + "<br><br>";
      else output = output + result + myLemma.getExplanation();
    }

    if (myLemma.getAttempts().size() > 0) output = output + s;
    output = output + "</html>";
