...
public class FSAToREController {
    ...
    protected int collapsesNeeded() {
        ...
        for (int i = 0; i < states.length; i++)
            for (int j = 0; j < states.length; j++)
                if (automaton.getTransitionsFromStateToState(states[i], states[j]).length > 1)
                    needed++;
        return needed;
    }
    ...
    protected int emptyNeeded() {
        ...
        for (int i = 0; i < states.length; i++)
            for (int j = 0; j < states.length; j++)
                if (automaton.getTransitionsFromStateToState(states[i], states[j]).length == 0)
                    needed++;
        return needed;
    }
    ...
    public void tableTransitionSelected(Transition transition) {
        ...
        Transition a = automaton.getTransitionsFromStateToState(from, collapseState)[0];
        Transition b = automaton.getTransitionsFromStateToState(from, to)[0];
        Transition c = automaton.getTransitionsFromStateToState(collapseState, collapseState)[0];
        Transition d = automaton.getTransitionsFromStateToState(collapseState, to)[0];
            ...
            case COLLAPSE_STATES:
                ...
                for (int i = 0; i < s.length; i++) {
                    if (automaton.getFinalStates()[0] == s[i]
                        ...
                        continue;
                    Transition[] t = FSAToRegularExpressionConverter
                            .getTransitionsForRemoveState(s[i], automaton);
