...
public class FSAToREController {
    ...
    protected void nextStep() {
        switch (currentStep) {
            ...
            case TRANSITIONS_TO_SINGLE_FINAL:
                detailStep.setText("Put " + Universe.curProfile.getEmptyString()
            ...
            case CONVERT_TRANSITIONS:
                ...
                detailStep.setText("Use the collapse tool to turn multiple transitions to one."
            ...
            case CREATE_EMPTY_TRANSITIONS:
                detailStep.setText("Put empty transitions between states with no transitions." + " "
                        + remaining + " more empty transitions needed.");
                if (remaining != 0) {
                    return;
                }
            ...
            case COLLAPSE_STATES:
                ...
                detailStep.setText("Use the collapse state tool to remove nonfinal, noninitial "
    ...
    public State stateCreate(Point point) {
        ...
        List<State> finals = automaton.getFinalStates();
        drawer.clearSelected();
        for (State si : finals) {
    ...
    public Transition transitionCreate(State from, State to) {
        if (currentStep == TRANSITIONS_TO_SINGLE_FINAL) {
            if (automaton.getFinalStates().get(0) != to) {
                JOptionPane.showMessageDialog(frame, "Transitions must go to the new final state!",
                        "Bad Destination", JOptionPane.ERROR_MESSAGE);
                return null;
            }
            if (!drawer.isSelected(from)) {
                JOptionPane.showMessageDialog(frame,
                        "Transitions must come from an old final state!", "Bad Source",
        ...
        if (currentStep == CREATE_EMPTY_TRANSITIONS) {
            if (automaton.getTransitionsFromStateToState(from, to).size() != 0) {
                JOptionPane.showMessageDialog(frame,
                        "Transitions must go between" + "states with no transitions!",
                        "Transition Already Exists", JOptionPane.ERROR_MESSAGE);
                return null;
            }
            Transition t = FSAToRegularExpressionConverter.addTransitionOnEmptySet(from, to,
                    automaton);
            remaining--;
            nextStep();
            frame.repaint();
    ...
    public Transition transitionCollapse(State from, State to) {
        ...
        List<Transition> ts = automaton.getTransitionsFromStateToState(from, to);
        if (ts.size() <= 1) {
            JOptionPane.showMessageDialog(frame, "Collapse requires 2 or more transitions!",
    ...
    public boolean stateCollapse(State state) {
        ...
        if (automaton.getInitialState() == state) {
            JOptionPane.showMessageDialog(frame, "The initial state cannot be removed!",
                    "Initial State Selected", JOptionPane.ERROR_MESSAGE);
            return false;
        }
        if (automaton.getFinalStates().get(0) == state) {
            JOptionPane.showMessageDialog(frame, "The final state cannot be removed!",
        ...
        transitionWindow.setTransitions(
                FSAToRegularExpressionConverter.getTransitionsForRemoveState(state, automaton));
    ...
    public void finalizeStateRemove() {
        if (collapseState == null) {
            JOptionPane.showMessageDialog(frame, "A valid state has not been selected yet!",
                    "No State Selected", JOptionPane.ERROR_MESSAGE);
            return;
        }
        FSAToRegularExpressionConverter.removeState(collapseState,
                transitionWindow.getTransitions(), automaton);
        remaining--;
        nextStep();
        collapseState = null;
        drawer.clearSelected();
    ...
    public void tableTransitionSelected(Transition transition) {
        ...
        Transition a = automaton.getTransitionsFromStateToState(from, collapseState).get(0);
        Transition b = automaton.getTransitionsFromStateToState(from, to).get(0);
        Transition c = automaton.getTransitionsFromStateToState(collapseState, collapseState)
                .get(0);
        Transition d = automaton.getTransitionsFromStateToState(collapseState, to).get(0);
    ...
    public void moveNextStep() {
        switch (currentStep) {
            case CREATE_SINGLE_FINAL:
                ...
                return;
            case TRANSITIONS_TO_SINGLE_FINAL:
                List<State> states = drawer.getSelected();
            ...
            case COLLAPSE_STATES:
                List<State> s = automaton.getStates();
                for (State si : s) {
                    if (automaton.getFinalStates().get(0) == si
                            || automaton.getInitialState() == si) {
                        continue;
                    }
                    List<Transition> t = FSAToRegularExpressionConverter
                            .getTransitionsForRemoveState(si, automaton);
                ...
                break;
            case FINISHED:
                JOptionPane.showMessageDialog(frame, "You're done.  Go away.", "You're Done!",
                        JOptionPane.ERROR_MESSAGE);
                return;
            default:
                JOptionPane.showMessageDialog(frame, "This shouldn't happen!  Notify Thomas.",
    ...
    public void export() {
        if (computedRE == null) {
            JOptionPane.showMessageDialog(frame, "The conversion has not yet finished.",
                    "Not Finished", JOptionPane.ERROR_MESSAGE);
            return;
        }
     ...
     */
    private static final int CREATE_SINGLE_FINAL = 0, TRANSITIONS_TO_SINGLE_FINAL = 1,
            CONVERT_TRANSITIONS = 2, CREATE_EMPTY_TRANSITIONS = 3, COLLAPSE_STATES = 4,
            FINISHED = 200;
}
