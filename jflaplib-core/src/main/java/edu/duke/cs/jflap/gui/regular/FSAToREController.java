...
public class FSAToREController {
  ...
  protected void outOfOrder() {
    JOptionPane.showMessageDialog(
        frame,
        "That action is inappropriate for this step!",
        "Out of Order",
  ...
  protected void nextStep() {
    switch (currentStep) {
      ...
      case CONVERT_TRANSITIONS:
        ...
        detailStep.setText(
            "Use the collapse tool to turn multiple transitions to one."
                + " "
                + remaining
      ...
      case CREATE_EMPTY_TRANSITIONS:
        detailStep.setText(
            "Put empty transitions between states with no transitions."
                + " "
                + remaining
  ...
  protected int collapsesNeeded() {
    ...
    for (State si : states)
      for (State sj : states)
        if (automaton.getTransitionsFromStateToState(si, sj).size() > 1) needed++;
    return needed;
  }
  ...
  protected int emptyNeeded() {
    ...
    for (State si : states)
      for (State sj : states)
        if (automaton.getTransitionsFromStateToState(si, sj).size() == 0) needed++;
    return needed;
  }
  ...
  public Transition transitionCollapse(State from, State to) {
    ...
    Transition t =
        FSAToRegularExpressionConverter.combineToSingleTransition(from, to, ts, automaton);
    remaining--;
    frame.repaint();
    nextStep();
    return t;
  ...
  public void finalizeStateRemove() {
    ...
    FSAToRegularExpressionConverter.removeState(
        collapseState, transitionWindow.getTransitions(), automaton);
    remaining--;
    nextStep();
    collapseState = null;
    drawer.clearSelected();
  ...
  public void tableTransitionSelected(Transition transition) {
    ...
    Transition c = automaton.getTransitionsFromStateToState(collapseState, collapseState).get(0);
  ...
  public void moveNextStep() {
    switch (currentStep) {
      ...
      case TRANSITIONS_TO_SINGLE_FINAL:
        ...
        for (State si : states) transitionCreate(si, finalState);
        break;
      case CONVERT_TRANSITIONS:
        {
      ...
      case COLLAPSE_STATES:
        ...
        for (State si : s) {
          if (automaton.getFinalStates().get(0) == si || automaton.getInitialState() == si)
            continue;
          List<Transition> t =
              FSAToRegularExpressionConverter.getTransitionsForRemoveState(si, automaton);
      ...
      case FINISHED:
        JOptionPane.showMessageDialog(
            frame, "You're done.  Go away.", "You're Done!", JOptionPane.ERROR_MESSAGE);
        return;
      default:
        JOptionPane.showMessageDialog(
            frame,
  ...
  public void export() {
    if (computedRE == null) {
      JOptionPane.showMessageDialog(
          frame, "The conversion has not yet finished.", "Not Finished", JOptionPane.ERROR_MESSAGE);
      return;
    }
  ...
  private static final int
      CREATE_SINGLE_FINAL = 0,
      TRANSITIONS_TO_SINGLE_FINAL = 1,
      CONVERT_TRANSITIONS = 2,
      CREATE_EMPTY_TRANSITIONS = 3,
      COLLAPSE_STATES = 4,
      FINISHED = 200;
}
