...
public class CombineAutomaton extends AutomatonAction {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        List<EnvironmentFrame> frames = Universe.frames();
        for (int i = 0; i < frames.size(); i++) {
            Environment env = frames.get(i).getEnvironment();
        ...
        // Prompt the user.
        int result = JOptionPane.showOptionDialog(Universe.frameForEnvironment(environment), combo,
                "Combine Two", JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null,
                null, null);
        if (result != JOptionPane.YES_OPTION && result != JOptionPane.OK_OPTION) {
            return;
        }

        Automaton other = (Automaton) ((EnvironmentFrame) combo.getSelectedItem()).getEnvironment()
                .getObject();
    ...
    private void add(Automaton newOne, Automaton other) {
        AutomatonDrawer d1 = new AutomatonDrawer(newOne), d2 = new AutomatonDrawer(other);
        ...
        List<Transition> otherTransitions = other.getTransitions();
        for (int i = 0; i < otherTransitions.size(); i++) {
            Transition t = otherTransitions.get(i);
            State from = otherToNew.get(t.getFromState()), to = otherToNew.get(t.getToState());
