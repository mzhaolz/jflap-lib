  ...
  public void actionPerformed(ActionEvent e) {
    ...
    for (int i = 0; i < frames.length; i++) {
      ...
      if (env == environment
          ...
          || environment.getObject().getClass() != env.getObject().getClass()) continue;
    ...
    if (combo.getItemCount() == 0) {
      JOptionPane.showMessageDialog(
          Universe.frameForEnvironment(environment), "No other automatons of this type around!");
      return;
    }
    // Prompt the user.
    int result =
    ...
    Automaton other =
        (Automaton) ((EnvironmentFrame) combo.getSelectedItem()).getEnvironment().getObject();
