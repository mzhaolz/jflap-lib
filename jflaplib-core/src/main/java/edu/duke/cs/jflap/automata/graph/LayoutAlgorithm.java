...
public abstract class LayoutAlgorithm<V> {
     ...
     */
    public static void shiftOntoScreen(Graph<?> graph, Dimension size, Dimension buffer,
        ...
        for (int i = 0; i < vertices.length; i++)
            graph.moveVertex(vertices[i],
                    new Point2D.Double(graph.pointForVertex(vertices[i]).getX() - minX,
        ...
        if (widthRatio > 1.0 || !scaleOnlyOverflow) {
            for (int i = 0; i < vertices.length; i++)
                graph.moveVertex(vertices[i],
                        new Point2D.Double(graph.pointForVertex(vertices[i]).getX() / widthRatio,
                                graph.pointForVertex(vertices[i]).getY()));
        }
        if (heightRatio > 1.0 || !scaleOnlyOverflow) {
            for (int i = 0; i < vertices.length; i++)
                graph.moveVertex(vertices[i],
                        new Point2D.Double(graph.pointForVertex(vertices[i]).getX(),
        ...
        for (int i = 0; i < vertices.length; i++)
            graph.moveVertex(vertices[i],
                    new Point2D.Double(graph.pointForVertex(vertices[i]).getX() + buffer.getWidth(),
