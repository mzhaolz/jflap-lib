...
public class TMTransition extends Transition {
  ...
  public TMTransition(State from, State to, String ntoRead, String ntoWrite, String ndirection) {
    this(
        from,
        to,
        Lists.newArrayList(ntoRead),
        Lists.newArrayList(ntoWrite),
  ...
  public TMTransition(
    ...
    super(from, to);
    checkArgument(
        toReadArray.size() != toWriteArray.size() || directionArray.size() != toReadArray.size(),
  ...
  protected void setRead(String stringToRead, int tape) {
    ...
    if (stringToRead.equals("!")) stringToRead = "!" + BLANK;
    if (stringToRead.length() != 1
        && !(stringToRead.startsWith("!")
            || stringToRead.equals("~")
            || stringToRead.indexOf("}") != -1)) {
