...
public class Minimizer {
   ...
   */
  public String getTerminalToSplit(List<State> group, Automaton automaton, DefaultTreeModel tree) {
  ...
  public boolean isSplittableOnTerminal(
      List<State> group, String terminal, Automaton automaton, DefaultTreeModel tree) {
  ...
  public boolean stateGoesToGroupOnTerminal(
      State state, List<State> temp, String terminal, Automaton automaton) {
    for (int k = 0; k < temp.size(); k++) {
      List<Transition> transitions = automaton.getTransitionsFromStateToState(state, temp.get(k));
   ...
   *         <CODE>terminal</CODE>.
   */
  public List<List<State>> getGroupsFromGroupOnTerminal(
      List<State> group, String terminal, Automaton automaton, DefaultTreeModel tree) {
    List<List<State>> list = new ArrayList<>();
    for (int k = 0; k < group.size(); k++) {
      if (group.get(k).getAutomaton() != automaton) System.err.println("BADNESS!  BADNESS!");
  ...
  public MinimizeTreeNode getTreeNodeForObject(
      DefaultTreeModel tree, MinimizeTreeNode root, List<State> group) {
    List<State> rootNode = root.getStates();
    if (rootNode == group) {
      return root;
    }
    for (int k = 0; k < root.getChildCount(); k++) {
      MinimizeTreeNode node =
          getTreeNodeForObject(tree, (MinimizeTreeNode) root.getChildAt(k), group);
   ...
   */
  public List<List<State>> splitOnTerminal(
      List<State> group, String terminal, Automaton automaton, DefaultTreeModel tree) {
  ...
  public void addTrapState(Automaton automaton) {
    if (!needsTrapState(automaton)) return;
  ...
  public DefaultTreeModel getInitializedTree(Automaton automaton) {
    ...
    for (int k = 0; k < states.size(); k++) {
      if (!automaton.isFinalState(states.get(k))) list.add(states.get(k));
    }
  ...
  public void addChildrenToParent(
      List<List<State>> children, MinimizeTreeNode parent, DefaultTreeModel tree) {
    int index = 0;
  ...
  public State getStateMappedToGroup(List<State> toGroup, Automaton minDfa) {
    ...
    for (int k = 0; k < states.size(); k++) {
      ...
      if (tempGroup == toGroup) return states.get(k);
    }
    return null;
  }
   ...
   *            the dfa being minimized.
   */
  public List<Transition> getTransitionsForState(
  ...
  public boolean containsTrapState(List<State> group) {
    for (int k = 0; k < group.size(); k++) {
      if (group.get(k) == TRAP_STATE) return true;
    }
    return false;
  }
