...
class BuilderController {
    ...
    private void determineTransitions() {
        ...
        for (int i = 0; i < states.length; i++)
            remainingTransitions
                    .addAll(minimizer.getTransitionsForState(states[i], minDfa, dfa, tree));
    }
    ...
    public void done() {
        ...
        if (remain != 0) {
            ...
            JOptionPane.showMessageDialog(view, remain + " transition" + (remain == 1 ? "" : "s")
                    + " remain " + "to be placed.\n" + "One comes from the state highlighted.");
            drawer.clearSelected();
            return;
        }
        JOptionPane.showMessageDialog(view, "The minimized automaton is fully built!\n"
