...
public class PDAConvertController extends ConvertController {
   ...
   *            for
   */
  public PDAConvertController(
      ConvertPane pane, SelectionDrawer drawer, PushdownAutomaton automaton) {
  ...
  protected Production[] getProductions(Transition transition) {
    return (Production[])
        converter
            .createProductionsForTransition(transition, getAutomaton())
  ...
  protected Grammar getGrammar() {
    ...
    while (it.hasNext()) grammar.addProduction((Production) it.next());
    return grammar;
  }
