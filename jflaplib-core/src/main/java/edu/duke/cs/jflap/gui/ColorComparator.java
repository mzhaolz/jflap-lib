...
public class ColorComparator implements Comparator<Color> {
	...
	public static void main(final String args[]) {
		final Color first = Color.black;
		final Color second = Color.red;
		final Comparator<Color> comp = new ColorComparator();
	...
	public int compare(final Color first, final Color second) {
		...
		final float[] firstHSB = Color.RGBtoHSB(first.getRed(), first.getGreen(), first.getBlue(), null);
		final float[] secondHSB = Color.RGBtoHSB(second.getRed(), second.getGreen(), second.getBlue(), null);
		final int[] comp = new int[3];
		// First saturation...
	 ...
	 *         larger
	 */
	private int compareFloat(final float float1, final float float2) {
		if (float1 > float2) {
			return 1;
		}
		if (float2 > float1) {
