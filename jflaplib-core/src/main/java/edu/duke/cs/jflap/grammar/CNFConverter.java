...
public class CNFConverter {
  ...
  public static Production[] convert(Production[] p) {
    ...
    for (int i = 0; i < p.length; i++) {
      ...
      for (int j = 0; j < tokens.length; j++)
        if (tokens[j].length() == 1) vars.remove(tokens[j]);
  ...
  public Production[] replacements(Production production) {
    ...
    for (int i = 0; i < tokens.length; i++)
      if (grammar.isTerminal(tokens[i])) return determinalize(production);
    ...
    if (leftAdded)
      return new Production[] {
        new Production(lhs, tokens[0] + left), new Production(left, remainder)
      };
  ...
  public Production[] determinalize(Production production) {
    ...
    for (int i = 0; i < tokens.length; i++) {
      if (grammar.isTerminal(tokens[i])) {
        ...
        if (leftAdded) list.add(new Production(newR, tokens[i]));
        rhs += newR;
      } else rhs += tokens[i];
    }
