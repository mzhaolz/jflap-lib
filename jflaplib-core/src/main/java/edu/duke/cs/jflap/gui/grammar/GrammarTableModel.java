...
public class GrammarTableModel extends GrowableTableModel<Object> {
  ...
  public GrammarTableModel(Grammar grammar) {
    ...
    for (int i = 0; i < ps.size(); i++) addProduction(ps.get(i));
  }
  ...
  public List<Production> getProductions() {
    ...
    for (int i = 0; i < getRowCount() - 1; i++) {
      ...
      if (production != null) list.add(production);
    }
    return list;
  }
  ...
  public Object getValueAt(int row, int column) {
    if (column == 1 && row == getRowCount() - 1) return null;
