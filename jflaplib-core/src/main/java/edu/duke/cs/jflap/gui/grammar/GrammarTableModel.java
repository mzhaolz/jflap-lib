...
public class GrammarTableModel extends GrowableTableModel<Object> {
	 ...
	 */
	public GrammarTableModel(final Grammar grammar) {
		this();
		final List<Production> ps = grammar.getProductions();
	 ...
	 */
	public int addProduction(final Production production) {
	 ...
	 */
	public int addProduction(final Production production, final int row) {
		final List<Object> o = initializeRow(row);
	 ...
	 * @return the production at this row, or <CODE>null</CODE> if what is there
	 *         is not properly a production
	 */
	public Production getProduction(final int row) {
		final String lhs = (String) getValueAt(row, 0);
		if (lhs.equals("")) {
			return null;
		}
	 ...
	 * not have a valid production, it is skipped. In this way, index
	 * <CODE>i</CODE> of the array does not necessarily correspond to row
	 * <CODE>i</CODE> of the model.
	 *
	...
	public List<Production> getProductions() {
		...
		for (int i = 0; i < getRowCount() - 1; i++) {
			final Production production = getProduction(i);
			if (production != null) {
				list.add(production);
			}
		}
	 ...
	 */
	@Override
	protected List<Object> initializeRow(final int row) {
	 ...
	 * @return <CODE>true</CODE> if and only if this is a column other than the
	 *         middle column, which is column index 1
	 */
	@Override
