...
public class DFAEqualityChecker {
	 ...
	 * instances of <CODE>FiniteStateAutomaton</CODE>, both are deterministic,
	 * and both have an initial state. Results are undefined otherwise.
	 *
	 * @param one
	 *            the first dfa
	 * @param two
	 ...
	 *         if they are not
	 */
	public boolean equals(final FiniteStateAutomaton one, final FiniteStateAutomaton two) {
		// Make sure they have the same number of states.
		if (one.getStates().size() != two.getStates().size()) {
			return false;
		}
	 ...
	 */
	private boolean hypothesize(final State state1, final State state2, final Map<State, State> matching) {
		{
			// Does state one already have a counterpart?
			final State counterpart = matching.get(state1);
		...
		final Map<String, Transition> labelToTrans1 = new HashMap<>(), labelToTrans2 = new HashMap<>();
		final List<Transition> t1 = state1.getAutomaton().getTransitionsFromState(state1);
		final List<Transition> t2 = state2.getAutomaton().getTransitionsFromState(state2);
		...
		for (int i = 0; i < t1.size(); i++) {
			final String label = ((FSATransition) t1.get(i)).getLabel();
			final Transition counterpart = labelToTrans2.get(label);
			// Does the same transition exist in the other automaton?
			if (counterpart == null) {
				return false;
			}
			matching.put(state1, state2);
			final boolean equal = hypothesize(t1.get(i).getToState(), counterpart.getToState(), matching);
			if (!equal) {
