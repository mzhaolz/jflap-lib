...
public class Torn {
     ...
     */
    public static GeneralPath getTorn(float x, float y, float width, float height, boolean left,
            boolean right) {
     ...
     */
    public static float paintString(Graphics2D g, String string, float x, float y, int align,
            float width, boolean left, boolean right, int select) {
        ...
        if (select >= 0 && !left && right) {
            ...
            if (select >= 0) {
                ...
                double c = metrics.getStringBounds(string.substring(select, select + 1), g2)
                        .getWidth();
