...
public class HumanCFPumpingLemmaInputPane extends HumanFirstPane {
    ...
    /**
     * Creates a <code>HumanCFPumpingInputPane</code> for a
    ...
    @Override
    protected void setI() {
        List<Integer> d = Lists.newArrayList(myUPanel.getVal(),
                myVPanel.getVal() - myUPanel.getVal(), myXPanel.getVal() - myVPanel.getVal(),
                myYPanel.getVal() - myXPanel.getVal(), myLemma.getW().length() - myYPanel.getVal());
    ...
    protected String createXYZ() {
        return "<i>uv</i><sup>" + myLemma.getI() + "</sup><i>xy</i><sup>" + myLemma.getI()
                + "</sup><i>z</i>";
    }
    ...
    public void update() {
         ...
         */
        List<Integer> decomposition = Lists.newArrayList(pl.getU().length(), pl.getV().length(),
                pl.getX().length(), pl.getY().length());

        /*
         * We need to call SliderPanel.setText and SliderPanel.setSliderMax
         * before calling setDecomposition. Otherwise, the sliders' default max
         * is 10, and if we try to set the value anything above 10, we will just
         * get 10.
         */
        ...
        /*
         * If the decomposition hasn't been set, i.e. the sliders are all set to
         * zero, don't allow setting of decomposition.
         *
         * Else, initialize animation etc.
         */
        if (pl.getU().length() == 0 && pl.getV().length() == 0 && pl.getX().length() == 0
                && pl.getY().length() == 0) {
            myDecompButton.setEnabled(false);
        } else {
