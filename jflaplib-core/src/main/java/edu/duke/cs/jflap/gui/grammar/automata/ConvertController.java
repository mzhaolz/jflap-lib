...
public abstract class ConvertController {
  ...
  protected void changeSelection() {
    ...
    for (; min <= max; min++) {
      if (!model.isSelectedIndex(min)) continue;
  ...
  protected void fillMap() {
    ...
    for (int i = 0; i < states.length; i++) {
      ...
      for (int j = 0; j < prods.length; j++) productionToObject.put(prods[j], states[i]);
    }
    ...
    for (int i = 0; i < transitions.length; i++) {
      ...
      for (int j = 0; j < prods.length; j++) productionToObject.put(prods[j], transitions[i]);
    }
  }
  ...
  public Object revealRandomProductions() {
    Iterator<Map.Entry<Serializable, Production[]>> it = objectToProduction.entrySet().iterator();
    while (it.hasNext()) {
  ...
  public Object[] highlightUntransformed() {
    HashSet<Serializable> unselectedSet = new HashSet<Serializable>(objectToProduction.keySet());
    unselectedSet.removeAll(alreadyDone);
    Object[] unselected = unselectedSet.toArray();
    drawer.clearSelected();
    for (int i = 0; i < unselected.length; i++)
      if (unselected[i] instanceof State) drawer.addSelected((State) unselected[i]);
