    ...
    switch (stepMode) {
      ...
      case NORMAL:
        ...
        if (pane.grammar.isVariable(top)) {
          ...
          derivationString =
              derivationString.substring(0, first) + ENTRY + derivationString.substring(first + 1);
          pane.derivationModel.addRow(new String[] {rule, derivationString});
          // What? About? Lambda?
          if (ENTRY.length() == 0) ENTRY = Universe.curProfile.getEmptyString();
      ...
      case REPLACING:
        ...
        pane.treePanel.repaint();
        if (!node.toString().equals(Universe.curProfile.getEmptyString())) STACK.push(node);
        ENTRYP--;
        updateStatus();
        return;
      case ERROR:
        ...
        if (entry == null) {
          ...
          if (entry.length() == 0) {
            MutableTreeNode child =
                new DefaultMutableTreeNode(Universe.curProfile.getEmptyString());
            node.insert(child, 0);
            nodes.add(child);
          } else {
            for (int i = entry.length() - 1; i >= 0; i--) {
              MutableTreeNode child = new DefaultMutableTreeNode(entry.substring(i, i + 1));
              node.insert(child, 0);
              stack.push(child);
              nodes.add(child);
            }
