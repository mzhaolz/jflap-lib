...
public class GEMLayoutAlgorithm<V> extends LayoutAlgorithm<V> {
	 ...
	 */
	public GEMLayoutAlgorithm(final Dimension pSize, final Dimension vDim, final double vBuffer) {
		super(pSize, vDim, vBuffer);
	}

	@Override
	public void layout(final Graph<V> graph, Set<V> isovertices) {
		if (isovertices == null) {
			isovertices = Collections.emptySet();
		}
		final List<V> vArray = graph.vertices();
		final int Rmax = 120 * (vArray.size() - isovertices.size());
		final double Tglobal = Tmin + 1.0;
		...
		if (isovertices.size() > 0) {
			int count = 0;
			double lengths = 0.0;
			final Iterator<V> it1 = isovertices.iterator();
			final Iterator<V> it2 = isovertices.iterator();
			while (it1.hasNext()) {
				while (it2.hasNext()) {
					final V v1 = it1.next();
					final V v2 = it2.next();
		...
		final double[] c = new double[] { 0.0, 0.0 };
		...
		for (int i = 0; i < Rmax && Tglobal > Tmin; i++) {
			...
			final V vertex = vertices.remove(index);
			final Record record = records.get(vertex);
			final Point2D point = graph.pointForVertex(vertex);
			...
			final double[] p = new double[] {
					...
					(c[1] / graph.numberOfVertices() - point.getY()) * GRAVITATIONAL_CONSTANT * Theta }; // Attraction
			// to
			// BC.
			// Random disturbance.
			...
			for (int j = 0; j < vArray.size(); j++) {
				if (vArray.get(i) == vertex) {
					continue;
				}
				final Point2D otherPoint = graph.pointForVertex(vArray.get(i));
				final double[] delta = new double[] { point.getX() - otherPoint.getX(),
						point.getY() - otherPoint.getY() };
				final double D2 = delta[0] * delta[0] + delta[1] * delta[1];
				final double O2 = optimalEdgeLength * optimalEdgeLength;
				if (delta[0] != 0.0 || delta[1] != 0.0) {
					for (int k = 0; k < 2; k++) {
						p[k] += delta[k] * O2 / D2;
					}
				}
				if (!graph.hasEdge(vertex, vArray.get(j))) {
					continue;
				}
				for (int k = 0; k < 2; k++) {
			...
			if (p[0] != 0.0 || p[1] != 0.0) {
				final double absp = Math.sqrt(Math.abs(p[0] * p[0] + p[1] * p[1]));
				for (int j = 0; j < 2; j++) {
