...
public class RestrictedBruteParser extends BruteParser {
	 ...
	 *            the target string
	 */
	public RestrictedBruteParser(final Grammar grammar, final String target) {
	...
	public boolean isPossibleDerivation(final String derivation) {
		...
		if (derivation.length() == 0) {
			...
			endBookend = !grammar.isVariable(derivation.substring(derivation.length() - 1, derivation.length()));
		}
