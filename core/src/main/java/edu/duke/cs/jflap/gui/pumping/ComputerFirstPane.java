...
public abstract class ComputerFirstPane extends PumpingLemmaInputPane {
	...
	public ComputerFirstPane(final PumpingLemma l, final String title) {
		super(l, title);
	...
	protected String addIGameFeatures() {
		...
		((JTextField) myIDisplay).addActionListener(ev -> {
			try {
				final int i = Integer.parseInt(myIDisplay.getText());
				if (!(i >= 0 && i != 1 && i <= 12)) {
				...
				if (myLemma.isInLang(myLemma.createPumpedString())) {
					myLemma.addAttempt(
							myLemma.getDecompositionAsString() + "; I = " + myLemma.getI() + "; <i>Failed</i>");
				} else {
				...
				final String error = "Please enter a positive integer in range [0, 2...12] for best results.";
	...
	protected String addWGameFeatures() {
		...
		((JTextField) myWDisplay).addActionListener(ev -> {
			for (int i = 3; i < stages.length; i++) {
				stages[i].setVisible(false);
			}
			final String w = myWDisplay.getText();
