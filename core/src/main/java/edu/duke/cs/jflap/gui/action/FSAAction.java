...
/**
 * The <CODE>FSAAction</CODE> is the general action that various controllers for
 * operators on finite state automatons should subclass. The only real change
 * from the <CODE>RestrictedAction</CODE> is that by default the
 * <CODE>.isAcceptable</CODE> method now only returns true if the object is an
 * instance of <CODE>FiniteStateAutomaton</CODE>.
 *
...
public abstract class FSAAction extends RestrictedAction {
	 ...
	 */
	public FSAAction(final String string, final Icon icon) {
		super(string, icon);
	}
}
