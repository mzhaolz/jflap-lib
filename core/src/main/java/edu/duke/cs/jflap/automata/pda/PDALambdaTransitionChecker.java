...
public class PDALambdaTransitionChecker extends LambdaTransitionChecker {
	...
	/**
	 * Returns true if <CODE>transition</CODE> is a lambda transition (i.e. all
	...
	public boolean isLambdaTransition(final Transition transition) {
		...
		final String input = trans.getInputToRead();
		final String toPop = trans.getStringToPop();
		final String toPush = trans.getStringToPush();
		if (input.equals(LAMBDA) && toPop.equals(LAMBDA) && toPush.equals(LAMBDA)) {
			return true;
		}
		return false;
	}
