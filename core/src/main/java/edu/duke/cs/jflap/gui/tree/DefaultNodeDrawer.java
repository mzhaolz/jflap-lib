...
public class DefaultNodeDrawer implements NodeDrawer {
	...
	public static final Shape NODE_SHAPE = new Ellipse2D.Float(-NODE_RADIUS, -NODE_RADIUS, NODE_RADIUS * 2.0f,
			NODE_RADIUS * 2.0f);

	/** A constant node size. */
	public static final Rectangle2D NODE_SIZE = new Rectangle((int) NODE_RADIUS, (int) NODE_RADIUS,
	...
	public void draw(final Graphics2D g, final TreeNode node) {
		...
		final Rectangle2D bounds = getBounds(g, s);
		// Draw the label for the node.
		final int dx = ((int) bounds.getWidth()) >> 1;
		final int dy = ((int) -bounds.getY()) >> 1;
		g.drawString(s, -dx, dy);
		// Restore the color.
		g.setColor(c);
	}

	public void draw(final Graphics2D g, final TreeNode node, final boolean dummy) {
		g.fill(new Ellipse2D.Float(-NODE_RADIUS, -NODE_RADIUS, NODE_RADIUS * 4.0f, NODE_RADIUS * 2.0f));
		final Color c = g.getColor();
		g.setColor(Color.black);
		g.draw(new Ellipse2D.Float(-NODE_RADIUS, -NODE_RADIUS, NODE_RADIUS * 4.0f, NODE_RADIUS * 2.0f));
		...
		final Rectangle2D bounds = getBounds(g, s);
		// Draw the label for the node.
		final int dx = ((int) bounds.getWidth()) >> 2;
		final int dy = ((int) -bounds.getY()) >> 1;
		g.drawString(s, -dx, dy);
		// Restore the color.
		g.setColor(c);
	 ...
	 */
	protected Rectangle2D getBounds(final Graphics2D g, final String string) {
	 ...
	 */
	@Override
	public Rectangle2D nodeSize(final TreeNode node) {
		return nodeSize();
	}
	 ...
	 */
	@Override
	public boolean onNode(final TreeNode node, final double x, final double y) {
