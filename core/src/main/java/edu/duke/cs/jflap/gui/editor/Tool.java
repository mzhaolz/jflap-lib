...
/**
 * The <CODE>Tool</CODE> abstract class is a type of input adapter for the pane
 * used to edit the view, and the automaton. The tool also has the ability to
 * draw on the view.
 */
public abstract class Tool extends SuperMouseAdapter {
	 ...
	 */
	protected static int findDominant(final String string, final char c) {
		final int index = string.indexOf(Character.toUpperCase(c));
		if (index != -1) {
			return index;
		}
	 ...
	 *
	 * @param view
	 *            the view the tool is in, useful for calling
	 *            <CODE>repaint</CODE>
	 * @param drawer
	 *            the drawer of the automaton
	 */
	public Tool(final AutomatonPane view, final AutomatonDrawer drawer) {
		this.view = view;
		this.drawer = drawer;
	...
	protected Icon getIcon() {
		final java.net.URL url = getClass().getResource("/ICON/default.gif");
	...
	public String getShortcutToolTip() {
		final String tip = getToolTip();
		final KeyStroke stroke = getKey();
		if (stroke == null) {
			return tip;
		}
		final int index = findDominant(tip, stroke.getKeyChar());
		if (index == -1) {
