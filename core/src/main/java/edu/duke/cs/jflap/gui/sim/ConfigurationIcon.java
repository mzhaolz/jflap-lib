...
/**
 * The <CODE>ConfigurationIcon</CODE> is a general sort of icon that can be used
 * to draw transitions. The <CODE>Icon</CODE> can then be added as the part of
 * any sort of object.
 *
...
public abstract class ConfigurationIcon implements Icon {
	 ...
	 */
	public ConfigurationIcon(final Configuration configuration) {
		this.configuration = configuration;
		automaton = configuration.getCurrentState().getAutomaton();
	}
	 ...
	 * already been drawn is defined by the static variables
	 * <CODE>RIGHT_STATE</CODE> and <CODE>BELOW_STATE</CODE>. By default this
	 * method paints the state.
	 *
	 * @param c
	...
	public void paintConfiguration(final Component c, final Graphics2D g, final int width, final int height) {
		...
		// // ARROW_DRAWER.draw(g);
		// } else {
		STATE_DRAWER.drawState(g, automaton, configuration.getCurrentState(), STATE_POINT);
		// }
	}
