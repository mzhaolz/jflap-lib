...
public class HumanCFPumpingLemmaInputPane extends HumanFirstPane {
	 ...
	 */
	public HumanCFPumpingLemmaInputPane(final ContextFreePumpingLemma l) {
	...
	protected void refresh() {
		try {
			...
			if (myUPanel.getVal() > myVPanel.getVal()) {
				...
				myZLength.setText("");
				myDecompButton.setEnabled(false);
			} else if ((myVPanel.getVal() - myUPanel.getVal()) + (myYPanel.getVal() - myXPanel.getVal()) < 1) {
				myZDisplay.setText("");
	...
	public void setDecomposition(final List<Integer> decomposition) {
		final int u = decomposition.get(0);
		final int v = decomposition.get(1);
		final int x = decomposition.get(2);
		final int y = decomposition.get(3);
	...
	@Override
	protected void setI() {
		final List<Integer> d = Lists.newArrayList(myUPanel.getVal(), myVPanel.getVal() - myUPanel.getVal(),
