...
public class Torn {
	 ...
	 */
	public static GeneralPath getTorn(final float x, final float y, final float width, final float height,
	 ...
	 * @param align
	 *            the alignment of the torn, as specified by either
	 *            <CODE>Torn.TOP</CODE> (i.e. <CODE>(x,y)</CODE> is the upper
	 *            left), <CODE>Torn.MIDDLE</CODE>, or <CODE>Torn.BOTTOM</CODE>.
	 * @param width
	 ...
	 */
	public static float paintString(final Graphics2D g, final String string, final float x, float y, final int align,
			final float width, final boolean left, final boolean right, int select) {
		// Convert the y coordinate into the alignment into the upper
		// left corner.
		final FontMetrics metrics = g.getFontMetrics();
		final float toBaseline = PADDING + metrics.getAscent();
		final float height = toBaseline + PADDING + metrics.getDescent();
		if (align == MIDDLE) {
			y -= height * 0.5f;
		}
		if (align == BOTTOM) {
		...
		final GeneralPath torn = getTorn(x, y, width, height, left, right);
		...
		// place.
		final float horizontalPadding = metrics.charWidth(' ');
		...
		if (select >= 0 && !left && right) {
			...
			final String before = string.substring(0, select), after = string.substring(select);
			final double bLength = metrics.getStringBounds(before, g2).getWidth();
			metrics.getStringBounds(after, g2).getWidth();
			final float aStart = bLength > width / 2.0f ? width / 2.0f : horizontalPadding + (float) bLength;
			final float bStart = aStart - (float) bLength;
			...
			if (select >= 0) {
				final double l = metrics.getStringBounds(string.substring(0, select), g2).getWidth();
				final double c = metrics.getStringBounds(string.substring(select, select + 1), g2).getWidth();
