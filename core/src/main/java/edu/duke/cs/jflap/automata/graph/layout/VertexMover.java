...
/**
 * This class contains a few simple vertex movement options that do not
 * individually need a separate class.
 */
public class VertexMover<V> extends LayoutAlgorithm<V> {
	 ...
	 */
	public VertexMover(final Dimension pSize, final Dimension vDim, final double vBuffer, final int c) {
	 ...
	 * constructor.
	 */
	public VertexMover(final int c) {
		super();
		command = c;
	}
		...
		if (command == HORIZONTAL_CENTER) {
			...
			for (int i = 0; i < vertices.size(); i++) {
				...
				graph.moveVertex(vertices.get(i),
						new Point2D.Double(size.getWidth() - point.getY(), size.getHeight() - point.getX()));
			}
		} else if (command == ROTATE) {
			double theta = 0;
			try {
				// Prompt for the degree input
				final String input = JOptionPane.showInputDialog("How many degrees to turn the automaton? \n"
						+ "(positive - clockwise, negative - counterclockwise)", new String("180"));
				theta = Double.parseDouble(input);
			} catch (final NumberFormatException e) {
				JOptionPane.showMessageDialog(null, "That is not a valid degree value.", "Invalid Input",
