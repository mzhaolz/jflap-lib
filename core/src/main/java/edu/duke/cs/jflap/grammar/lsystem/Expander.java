...
public class Expander {
	...
	private class Context {
		 ...
		 */
		public Context(final List<String> tokens, final int center, final List<List<String>> replacements) {
			this.tokens = tokens;
			this.center = center;
		 ...
		 */
		public List<List<String>> matches(final List<String> list, int centerList) {
			centerList -= center;
			try {
				final List<String> sub = list.subList(centerList, centerList + tokens.size());
	...
	private final Random stochiastic;

	/** The cached expansions. At index 0 is the axiom. */
	private final List<List<String>> cachedExpansions = new ArrayList<>();
	 ...
	 */
	public Expander(final LSystem lsystem) {
	 ...
	 */
	public Expander(final LSystem lsystem, final long seed) {
	 ...
	 */
	private List<String> expand(final List<String> symbols) {
		if (contexts == null) {
	 ...
	 * @return the expansion of the passed in symbols
	 */
	private List<String> expandContext(final List<String> symbols) {
		final List<String> ne = new ArrayList<>();
		for (int i = 0; i < symbols.size(); i++) {
			final String s = symbols.get(i);
			final List<List<String>> replacementsList = new ArrayList<>();
			for (int j = 0; j < contexts.size(); j++) {
				final List<List<String>> l = contexts.get(j).matches(symbols, i);
				for (int k = 0; k < l.size(); k++) {
					replacementsList.add(l.get(k));
				}
			}
			final List<List<String>> replacements = replacementsList;
			...
			default:
				...
				// nearly at random.
				replacement = replacements.get(stochiastic.nextInt(replacements.size()));
				break;
			}
			final Iterator<String> it2 = replacement.iterator();
			while (it2.hasNext()) {
	 ...
	 * @return the expansion of the passed in symbols
	 */
	private List<String> expandNoContext(final List<String> symbols) {
		final List<String> ne = new ArrayList<>();
		for (int i = 0; i < symbols.size(); i++) {
			final String s = symbols.get(i);
			final List<List<String>> replacements = lsystem.getReplacements(s);
			List<String> replacement = null;
			switch (replacements.size()) {
			case 0:
				// This cannot be replaced, so we skip to the next symbol.
	...
	private final void initializeContexts() {
		final Iterator<String> symbolIt = lsystem.getSymbolsWithReplacements().iterator();
		final ArrayList<Context> contextsList = new ArrayList<>();
		boolean hasContexts = false;
		// Build the contexts.
		while (symbolIt.hasNext()) {
			final String symbol = symbolIt.next();
			List<String> tokens = LSystem.tokenify(symbol);
			final List<List<String>> replacements = lsystem.getReplacements(symbol);
			int context = 0;
			switch (tokens.size()) {
			case 0:
