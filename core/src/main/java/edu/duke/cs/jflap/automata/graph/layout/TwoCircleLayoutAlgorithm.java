...
/**
 * This layout algorithm arranges vertices according to a circle algorithm.
 * Vertices with a relatively high degree are placed in an inner circle. Those
 * with lower degrees are placed in an outer circle. Outer circle vertices that
 * link to a inner circle vertex are placed near that vertex in the outer
 * circle. All vertices are moved, despite any that may be flagged as
 * nonmovable.
 *
 * @see LayoutAlgorithm
 * @author Chris Morgan
...
public class TwoCircleLayoutAlgorithm<V> extends LayoutAlgorithm<V> {
	/**
	 * Some test code used to test out layout algorithms graphically without
	 * goint through JFLAP.
	 */
	public static void main(final String[] args) {
		...
		// Point2D.Double center = new Point2D.Double(f.getWidth()/2,
		// f.getHeight()/2);
		final String[] vertices = new String[20];
		final Graph<String> graph = new Graph<>();
		final LayoutAlgorithm<String> layout = new RandomLayoutAlgorithm<>();
		...
		/*
		 * graph.addEdge(vertices[0], vertices[1]); graph.addEdge(vertices[2],
		 * vertices[0]); graph.addEdge(vertices[1], vertices[2]);
		 * graph.addEdge(vertices[3], vertices[1]); graph.addEdge(vertices[3],
		 * verticesadjacent(finalStates[i]).size()[0]);
		...
		for (final String vertice : vertices) {
			final Point2D p2d = graph.pointForVertex(vertice);
			p.getGraphics().drawString(vertice, (int) (p2d.getX()), (int) (p2d.getY()) - 10);
			p.getGraphics().fillRect((int) (p2d.getX()), (int) (p2d.getY()), 30, 30);
		}
		for (final String vertice : vertices) {
			for (final String vertice2 : vertices) {
				if (graph.hasEdge(vertice, vertice2)) {
					p.getGraphics().drawLine((int) (graph.pointForVertex(vertice).getX()),
							(int) (graph.pointForVertex(vertice).getY()), (int) (graph.pointForVertex(vertice2).getX()),
							(int) (graph.pointForVertex(vertice2).getY()));
				}
			}
		}
		p.revalidate();
	 ...
	 * <code>innerCircleChain</code>, and values in each
	 * <code>outerCircleChain</code> are laid up opposite to its corresponding
	 * vertex. The <code>outerCircleChains</code>, when laid out on the graph,
	 * do not necessarily have the same radius in their layouts, as this varies
	 * based on the number of vertices in each <code>outerCircleChain</code>.
	 */
	 ...
	 */
	public TwoCircleLayoutAlgorithm(final Dimension pSize, final Dimension vDim, final double vBuffer) {
		super(pSize, vDim, vBuffer);
	}

	/**
	 * Divides the vertices by placing them either in the inner circle or outer
	 * circle. All inner circle vertices will have a degree greater than 2 or
	 * will be adjacent to two other inner circle vertices. The rest of the
	 * vertices are placed in the outer circles.
	 */
	...
	/**
	 * Divides the vertices that are in the outer circle into
	 * <code>VertexChains</code>, which correspond to an inner circle vertex.
	 * Outer circle vertices are assigned to a specific <code>VertexChain</code>
	 * according to the following priorities. <br>
	 * <br>
	 * 1. Whether they link to an inner circle vertex <br>
	 * 2. Whether they link to an existing outer circle item <br>
	 * 3. If they link to two outer circle items in different
	 * <code>VertexChains</code> or have a degree = 0, they are assigned to the
	 * smaller of the two <code>VertexChains</code> or the smallest existing
	 * <code>VertexChain</code>, respectively.
	 */
	@SuppressWarnings("unchecked")
	protected void createOuterCircleChains() {
		outerCircleChains = new CircleChain[innerCircle.size()];
		final int[] chainIndex = new int[outerCircle.size()];
		for (int i = 0; i < outerCircleChains.length; i++) {
			outerCircleChains[i] = new CircleChain<>(graph, vertexDim, vertexBuffer);
		}

		// First add the outerCircle vertices linking to an innerCircle vertex
		...
		// Next, if a vertex is linked to an outercircle item, add it; if to
		// items in two different chains, add
			...
			for (int i = 0; i < outerCircle.size(); i++) {
				if (chainIndex[i] == -1) {
					...
					if (match1 > -1 && match2 == -1) {
						...
						if (outerCircleChains[match1].size() < outerCircleChains[match2].size()) {
							min = match1;
						} else {
							min = match2;
						}
		...
		// If no vertex can be added, find the chain with minimum length, and
		// then add all unplaceded vertices to it.
		min = 0;
		for (int i = 0; i < outerCircleChains.length; i++) {
	...
	public void layout(final Graph<V> g, final Set<V> notMoving) {
		...
		// Put in the inner circle all vertices with degree >= 3 and all those
		// pointing to two members of the inner circle.
		assignToCircles();
		...
		innerCircle = innerCircleChain.getVertices(); // getting correct order
		// for outer circle chains
		...
		// Finally, adjust the points so that they can be presented on the
		// screen.
	...
	/**
	 * Shuffles the order of vertices in the outer circle
	 * <code>CircleChains</code> in order to minimize overlapping transitions
	 * between vertices in two different <code>CircleChains</code>. If a vertex
	 * from one <code>CircleChain
	 * </code> has an edge to another vertex in an adjacent
	 * <code>CircleChain</code>, the two vertices will be moved to their
	 * <code>CircleChain's</code> common border, and whatever other vertices to
	 * which the two vertices are linked will be adjusted accordingly.
	 */
